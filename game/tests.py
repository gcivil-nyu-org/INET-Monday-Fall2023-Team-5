...
class CharacterCreationViewTest(TestCase):
    ...
    def test_transition_to_regular_turn(self):
        # Create a game session in CHARACTER_CREATION state
        game_session = GameSession.objects.create(state=GameSession.CHARACTER_CREATION)

        # Create two players
        user1 = User.objects.create_user(username="player1", password="12345")
        user2 = User.objects.create_user(username="player2", password="12345")
        player1 = Player.objects.create(
            user=user1,
            game_session=game_session,
            character_creation_state=Player.PUBLIC_PROFILE_CREATION,
        )
        ...
        # Create a Character instance and associate it with players
        character = Character.objects.create(
            name="Test Character",
            description="Test Description",
        ...
        # Create necessary objects for PublicProfileCreationForm
        quality1 = Quality.objects.create(name="Quality 1")
        quality2 = Quality.objects.create(name="Quality 2")
        interest1 = Interest.objects.create(name="Interest 1")
        activity1 = Activity.objects.create(name="Activity 1")
        # Set choices for the character
        character.quality_1_choices.set([quality1, quality2])
        character.interest_1_choices.set([interest1])
        character.activity_1_choices.set([activity1])
        character.save()

        # Define post data for both players
        post_data = {
            "quality_1": quality1.id,
            "quality_2": quality2.id,
            "quality_3": quality2.id,  # Assuming you can select the same quality
            "interest_1": interest1.id,
            "interest_2": interest1.id,  # Assuming you can select the same interest
            "interest_3": interest1.id,
            "activity_1": activity1.id,
            "activity_2": activity1.id,  # Assuming you can select the same activity
        }

        # Log in as player1 and submit the form
        self.client.force_login(user1)
        response_player1 = self.client.post(
            reverse("character_creation", kwargs={"game_id": game_session.game_id}),
            post_data,
        )

        # Check response for player1
        self.assertEqual(response_player1.status_code, 302)
        if response_player1.context:
            form = response_player1.context.get("form")
            if form and not form.is_valid():
                print("Player 1 Form errors:", form.errors)

        player1.refresh_from_db()

        # Log in as player2 and submit the form
        self.client.force_login(user2)
        response_player2 = self.client.post(
            reverse("character_creation", kwargs={"game_id": game_session.game_id}),
            post_data,
        )

        # Check response for player2
        self.assertEqual(response_player2.status_code, 302)
        if response_player2.context:
            form = response_player2.context.get("form")
            if form and not form.is_valid():
                print("Player 2 Form errors:", form.errors)
