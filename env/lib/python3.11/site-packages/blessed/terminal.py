...
class Terminal(object):
    ...
    def __init__(self, kind=None, stream=None, force_styling=False):
        ...
        global _CUR_TERM
        self.errors = [
            "parameters: kind=%r, stream=%r, force_styling=%r"
    ...
    def __init__streams(self):
        ...
        if not hasattr(self._stream, "fileno"):
            ...
            try:
                ...
                self.errors.append(
                    "Unable to determine output stream file descriptor: %s" % err
                )
            else:
        ...
        if self._stream in (sys.__stdout__, sys.__stderr__):
            try:
                ...
                self.errors.append(
                    "Unable to determine input stream file descriptor: %s" % err
                )
            else:
        ...
        if stream_fd is None:
            try:
                ...
                self.errors.append(
                    "Unable to determine __stdout__ file descriptor: %s" % err
                )
    ...
    def __init__keycodes(self):
        ...
        if self._keyboard_fd is not None:
            ...
            try:
                ...
                warnings.warn(
                    "LookupError: {0}, defaulting to UTF-8 for keyboard.".format(err)
                )
    ...
    def get_location(self, timeout=None):
        ...
        response_str = (
            getattr(self, self.caps["cursor_report"].attribute) or "\x1b[%i%d;%dR"
        )
    ...
    def get_fgcolor(self, timeout=None):
        ...
        match = self._query_response(
            "\x1b]10;?\x07",
            re.compile("\x1b]10;rgb:([0-9a-fA-F]+)/([0-9a-fA-F]+)/([0-9a-fA-F]+)\x07"),
            timeout,
        )
    ...
    def get_bgcolor(self, timeout=None):
        ...
        match = self._query_response(
            "\x1b]11;?\x07",
            re.compile("\x1b]11;rgb:([0-9a-fA-F]+)/([0-9a-fA-F]+)/([0-9a-fA-F]+)\x07"),
            timeout,
        )
    ...
    def inkey(self, timeout=None, esc_delay=0.35):
        ...
        resolve = functools.partial(
            resolve_sequence, mapper=self._keymap, codes=self._keycodes
        )
