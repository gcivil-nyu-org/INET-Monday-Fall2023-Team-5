...
class DataPoller(object):
    ...
    def _get_health_data(self):
        ...
        data = {"environment": environment_health, "instances": instance_health}
        ...
        while True:
            try:
                ...
                if str(e) == responses["health.nodescribehealth"]:
                    ...
                    LOG.debug(
                        "Swallowing 'DescribeEnvironmentHealth is not supported' exception"
                    )
...
def collapse_instance_health_data(instances_health):
    ...
    for i in instance_list:
        instance = dict()
        request_count = i.get("ApplicationMetrics", {}).get("RequestCount", 0)
        ...
        if i.get("AvailabilityZone"):
            try:
                instance["AvailabilityZone"] = i.get("AvailabilityZone").rsplit("-", 1)[
                    -1
                ]
            except:
        ...
        instance["load1"] = (
            instance["LoadAverage"][0] if "LoadAverage" in instance else "-"
        )
        instance["load5"] = (
            instance["LoadAverage"][1] if "LoadAverage" in instance else "-"
        )
        ...
        for key in {"Status_2xx", "Status_3xx", "Status_4xx", "Status_5xx"}:
            _convert_data_to_percentage(
                instance, key, request_count, add_sort_column=True
            )
...
def _format_latency_dict(latency_dict, request_count):
    ...
    for k, v in six.iteritems(latency_dict):
        ...
        if (k == "P99" and request_count < 100) or (k == "P90" and request_count < 10):
            representation += "*"
        elif k in ["P99", "P90"]:
            representation += " "
