...
class Table(object):
    ...
    def draw(self, num_of_rows, table_data):
        ...
        self.first_column = min(
            self.screen.horizontal_offset + 1, len(self.columns) - 1
        )
    ...
    def draw_header_row(self):
        ...
        for c in [0] + list(range(self.first_column, len(self.columns))):
            ...
            if (
                self.screen.sort_index
                and self.screen.sort_index[1] == c
                and self.name == self.screen.sort_index[0]  # Sort column
                and len(" ".join(labels)) < width  # sort table
            ):  # Column is on screen
        ...
        if header_text[-Table.HEADER_SPACE_NEEDED :].isspace():
            header_text = (
                header_text[: -Table.HEADER_SPACE_NEEDED]
                + "  {n}{b} "
    ...
    def draw_info_line(self, first_row_index, last_row_index):
        ...
        if last_row_index < len(self.data):
            # Show down arrow
            line += " {}".format(term.DOWN_ARROW)
        else:
            line += "  "
        if first_row_index != 0:
            # Show up arrow
            line += " {}".format(term.UP_ARROW)
        term.echo_line(line)
    ...
    def last_row_index(self):
        return min(len(self.data), self.visible_rows + self.vertical_offset)

    def get_row_data(self, data):
        row_data = [self.get_color_data(data)]
        for c in [0] + list(range(self.first_column, len(self.columns))):
            column = self.columns[c]
    ...
    if justify_side == "right":
        ...
        if key is not None and key == "Description":
            if len(s) > justify_size:
                s = _add_arrow(s, justify_size - 1, "\u25B6", ">")
            if shift_col > 0:
                s = _add_arrow(s, 0, "\u25C0", "<", s[1:])
