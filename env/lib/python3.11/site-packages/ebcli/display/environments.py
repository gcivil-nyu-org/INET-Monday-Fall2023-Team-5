...
class EnvironmentScreen(Screen):
    ...
    def show_help_line(self):
        text = " (Commands: {q}uit, {r}estore, {down} {up})".format(
            q=io.bold("Q"), r=io.bold("R"), down=term.DOWN_ARROW, up=term.UP_ARROW
        )
        term.echo_line(text)
    ...
    def restore(self, t):
        ...
        save = self.prompt_and_action(
            prompts["restore.prompt"], self.restore_environment_num
        )
        self.flusher(t)
        return save
    ...
    def restore_environment_num(self, environment_number):
        ...
        environment_number = int(
            environment_number
        )  # raises InvalidOperation Exception
...
class EnvironmentDataPoller(DataPoller):
    ...
    def prep_version_data(self, page_table_data):
        ...
        for x in range(page_range):
            ...
            if isinstance(page_table_data[x]["DateUpdated"], datetime):
                page_table_data[x]["SinceCreated"] = format_time_since(
                    page_table_data[x]["DateUpdated"]
                )
                page_table_data[x]["DateUpdated"] = get_local_time(
                    page_table_data[x]["DateUpdated"]
                ).strftime("%Y/%m/%d %H:%M %Z")
            else:
                page_table_data[x]["SinceCreated"] = None
                page_table_data[x]["DateUpdated"] = None
    ...
    def get_table_data(self):
        return {TABLE_DATA_KEY: self.environments}
