...
class TraditionalHealthDataPoller(DataPoller):
    ...
    def get_health_information_of_instance_not_associated_with_elb(
        ...
        for instance_id in list(
            ids_of_all_instances - ids_of_instances_registered_with_elb
        ):
    ...
    def get_load_balancer_instance_states(self, load_balancer_name):
        if elb.is_classic_load_balancer(load_balancer_name):
            ...
            target_group_arns = [
                target_group["TargetGroupArn"] for target_group in elbv2_target_groups
            ]
            instance_states = elbv2.get_instance_healths_from_target_groups(
                target_group_arns
            )
    ...
    def _get_health_data(self):
        ...
        instances_registered_with_elb = self.get_instance_states(
            environment_resources.get("LoadBalancers")
        )
        ids_of_all_instances = [
            instance["Id"] for instance in environment_resources.get("Instances", [])
        ]
        all_instances = self.get_instance_health(
            instances_registered_with_elb
        ) + self.get_health_information_of_instance_not_associated_with_elb(
            ids_of_all_instances, instances_registered_with_elb
        )

        return {
            "instances": all_instances,
            "environment": self.assemble_environment_data(
                ids_of_all_instances, instances_registered_with_elb
            ),
        }
...
class TraditionalHealthScreen(Screen):
    ...
    def draw_banner_info_lines(self, lines, data):
        if lines > 2:
            term.echo_line(
                "instances:",
                io.bold(data.get("Total", 0)),
                "Total,",
                io.bold(data.get("InService", 0)),
                "InService,",
                io.bold(data.get("Other", 0)),
                "Other",
            )
            lines -= 1
        if lines > 2:
