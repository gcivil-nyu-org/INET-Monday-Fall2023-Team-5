...
class Screen(object):
    ...
    def start_screen(
        self, poller, env_data, refresh, mono=False, default_table="split"
    ):
        self.mono = mono
    ...
    def draw(self, key):
        ...
        if visible_count != 0:
            ...
            if visible_rows > 0:  # Dont show tables if no visible rows.
                self.max_columns = (
                    max([len(t.columns) for t in self.tables if t.visible]) - 1
                )
                for table in self.tables:
    ...
    def snapshot_file_view(self):
        ...
        with t.location(y=self.empty_row, x=2):
            io.echo(
                io.bold("Snapshot file saved at: .elasticbeanstalk/" + filename),
                end=" ",
            )
            sys.stdout.flush()
            time.sleep(4)
        ...
        with t.location(y=self.empty_row, x=2):
            ...
            try:
                ...
                LOG.debug(
                    ...
                    "and the request could not be completed.".format(type(e), str(e))
                )
                io.log_error(
    ...
    def replace_instance_view(self):
        self.prompt_and_action("instance-ID to replace:", ec2.terminate_instance)
    ...
    def draw_banner_first_line(self, lines, data):
        ...
        pad_length = term.width() - len(env_name) - len(timestamp) - len(countdown) - 1
        if lines > 2:
            banner = io.bold(" {env_name}{status}{time}{cd} ").format(
                env_name=env_name,
    ...
    def draw_banner_info_lines(self, lines, data):
        ...
        if lines > 3:
            ...
            term.echo_line("".join((s.center(column_size) for s in instance_counts)))
            term.echo_line(
                "".join(
                    (
    ...
    def show_help_line(self):
        if self.help_table.visible:
            ...
            text = " (Commands: {h}elp,{q}uit, {down} {up} {left} {right})".format(
                h=io.bold("H"),
                q=io.bold("Q"),
    ...
    def sort_data(self, data):
        ...
        if self.sort_index:
            ...
            new_data["instances"].sort(
                key=lambda x: x.get(sort_key, "-"), reverse=self.sort_reversed
            )
        return new_data
