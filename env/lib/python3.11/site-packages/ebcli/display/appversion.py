...
class VersionScreen(Screen):
    ...
    def draw_banner_first_line(self, lines, data):
        if lines > 2:
            ...
            banner = io.bold(" {env_name}{app_name} ").format(
                env_name=env_name,
    ...
    def draw_banner_info_lines(self, lines, data):
        if lines > 2:
            ...
            term.echo_line(
                ...
                io.bold(status),
                "Health",
                io.bold(data.get("Color", "Unknown")),
            )
            lines -= 1
        if lines > 2:
            term.echo_line(
    ...
    def show_help_line(self):
        text = " (Commands: {q}uit, {d}elete, {l}ifecycle, {down} {up} {left} {right})".format(
            q=io.bold("Q"),
            d=io.bold("D"),
            l=io.bold("L"),
    ...
    def handle_input(self):
        ...
        with t.cbreak():  # Get input
            ...
            if val:
                char = str(val).upper()
                LOG.debug("Got val: {0}, {1}, {2}.".format(val, val.name, val.code))
                if char == "Q":  # Quit
    ...
    def scroll_over(self, reverse=False):
        ...
        if reverse and table.shift_col > 0:
            ...
            if (
                table.get_widest_data_length_in_column(table.columns[4])
                > self.MAX_SHIFT - 10
            ):
    ...
    def delete_app_version_num(self, version_number):
        ...
        version_label = app_version.get("VersionLabel")

        from ebcli.operations import appversionops

        should_exit_table = appversionops.delete_app_version_label(
            self.poller.app_name, version_label
        )
        time.sleep(4)
...
class VersionDataPoller(DataPoller):
    def __init__(self, app_name, env_name, all_app_versions):
        ...
        if self.env_name is not None:
            ...
            self.env = elasticbeanstalk.get_environment(
                app_name=self.app_name, env_name=self.env_name
            )
    ...
    def prep_version_data(self, page_table_data):
        ...
        for x in range(page_range):
            ...
            if isinstance(page_table_data[x]["DateCreated"], datetime):
                page_table_data[x]["SinceCreated"] = format_time_since(
                    page_table_data[x]["DateCreated"]
                )
                page_table_data[x]["DateCreated"] = get_local_time(
                    page_table_data[x]["DateCreated"]
                ).strftime("%Y/%m/%d %H:%M")
            else:
                page_table_data[x]["SinceCreated"] = None
                page_table_data[x]["DateCreated"] = None
    ...
    def get_table_data(self):
        return {"environment": self.env_data, "app_versions": self.app_versions}

    def get_curr_deploy_num(self):
        curr_deploy_num = 0
        for x in range(len(self.all_app_versions)):
            if self.all_app_versions[x]["VersionLabel"] == self.env.version_label:
