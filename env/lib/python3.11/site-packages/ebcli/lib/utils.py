...
def list_to_columns(lst):
    COLUMN_NUM = 3
    assert len(lst) > COLUMN_NUM, "List size must be greater than {0}".format(
        COLUMN_NUM
    )
...
def check_source(value):
    ...
    if match is None or len(value.split("/")) > 3:
        raise argparse.ArgumentTypeError(
            "%s is a invalid source. Example source would be something like: codecommit/repo/branch"
            % value
        )
    return value
...
def raise_if_source_location_is_not_codecommit(source_location):
    if source_location != "codecommit":
        raise InvalidOptionsError(
            'Source location "{0}" is not supported by the EBCLI'.format(
                source_location
            )
        )
...
def padded_list(candidate, reference_list):
    ...
    if not reference_list:
        raise AttributeError(
            "The reference_list argument must be non-empty.".format(reference_list)
        )
...
def row_wrapper(string_width_mappings, padding=3):
    ...
    for row in range(len(longest_column)):
        __wrapped_strings += [
            (" " * padding).join([column[row] for column in wrapped_strings])
        ]
