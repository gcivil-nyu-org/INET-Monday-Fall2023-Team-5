...
def upload_file(bucket, key, file_path):
    with open(file_path, "rb") as fp:
        return _make_api_call("put_object", Bucket=bucket, Key=key, Body=fp)
...
def bucket_exists(bucket):
    ...
    return bucket in [
        bucket["Name"] for bucket in _make_api_call("list_buckets")["Buckets"]
    ]


def get_object_info(bucket, object_key):
    result = _make_api_call("list_objects", Bucket=bucket, Prefix=object_key)
    ...
    if len(objects) == 1:
        ...
        s3_object = next(
            (s3_object for s3_object in objects if s3_object["Key"] == object_key), None
        )
...
def get_object(bucket, key):
    result = _make_api_call("get_object", Bucket=bucket, Key=key)
...
def multithreaded_upload(bucket, key, file_path):
    ...
    try:
        ...
        with open(file_path, "rb") as f:
            ...
            for i in range(THREAD_COUNT):
                p = threading.Thread(
                    ...
                    args=(f, lock, etaglist, total_parts, bucket, key, upload_id),
                )
                p.daemon = True
                jobs.append(p)
                p.start()
        ...
        if not _all_parts_were_uploaded(etaglist, total_parts):
            LOG.debug(
                "Uploaded {0} parts, but should have uploaded {1} parts.".format(
                    len(etaglist), total_parts
                )
            )
            raise UploadError(
                "An error occured while uploading Application Version. "
...
def _get_part_etag(bucket, key, part, upload_id):
    try:
        response = _make_api_call(
            "list_parts", Bucket=bucket, Key=key, UploadId=upload_id
        )
    except Exception as e:
    ...
    if "Parts" not in response:
        return None
    etag = next((i["ETag"] for i in response["Parts"] if i["PartNumber"] == part), None)
    return etag


def _get_multipart_upload_id(bucket, key):
    # Check to see if multipart already exists
    response = _make_api_call("list_multipart_uploads", Bucket=bucket, Prefix=key)
    ...
    response = _make_api_call("create_multipart_upload", Bucket=bucket, Key=key)
