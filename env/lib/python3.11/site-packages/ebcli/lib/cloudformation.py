    ...
    while True:
        if next_token:
            result = _make_api_call(
                "describe_stack_events", StackName=stack_name, NextToken=next_token
            )
        else:
...
def get_template(stack_name):
    result = _make_api_call("get_template", StackName=stack_name)
    return result
    ...
    while not stack_exists:
        if (datetime.now() - start_time) > timedelta(timeout):
            raise CFNTemplateNotFound(
                "Could not find CFN stack, '{stack_name}'.".format(
