...
def _get_data_loader():
    ...
    data_folder = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), BOTOCORE_DATA_FOLDER_NAME
    )

    return Loader(
        extra_search_paths=[data_folder, Loader.BUILTIN_DATA_PATH],
    ...
    while True:
        ...
        try:
            ...
            raise CredentialsError(
                "Operation Denied. You appear to have no" " credentials"
            )
            ...
            if (
                hasattr(error.args[0], "reason")
                and str(error.args[0].reason) == "[Errno -2] Name or service not known"
            ):
...
def _handle_response_code(response_data, attempt, aggregated_error_message):
    ...
    if status == 400:
        ...
        if isinstance(error, ThrottlingError):
            ...
            if attempt > max_attempts:
                raise MaxRetriesError("Max retries exceeded for " "throttling error")
        else:
            raise error
    elif status == 403:
        ...
        raise ServiceError(
            "API Call unsuccessful. " "Status code returned " + str(status)
        )
...
def _set_operation(service_name, operation_name):
    try:
        ...
        LOG.debug("Credentials incomplete")
        raise CredentialsError(
            "Your credentials are not complete. Error: {0}".format(e)
        )
