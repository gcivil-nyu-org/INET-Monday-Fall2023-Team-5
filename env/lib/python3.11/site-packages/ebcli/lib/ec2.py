...
from ebcli.objects.exceptions import ServiceError, AlreadyExistsError, NotFoundError
...
def import_key_pair(keyname, key_material):
    try:
        result = _make_api_call(
            "import_key_pair", KeyName=keyname, PublicKeyMaterial=key_material
        )
...
def describe_instances(instance_ids):
    result = _make_api_call("describe_instances", InstanceIds=instance_ids)
...
def has_default_vpc():
    result = _make_api_call(
        "describe_account_attributes", AttributeNames=["default-vpc"]
    )
    default_vpc = None
...
def describe_security_group(security_group_id):
    result = _make_api_call("describe_security_groups", GroupIds=[security_group_id])
    if result and len(result["SecurityGroups"]) < 1:
        raise NotFoundError("Security Group {} not found.".format(security_group_id))
    return result["SecurityGroups"][0]


def terminate_instance(instance_id):
    return _make_api_call("terminate_instances", InstanceIds=[instance_id])


def reboot_instance(instance_id):
    return _make_api_call("reboot_instances", InstanceIds=[instance_id])
