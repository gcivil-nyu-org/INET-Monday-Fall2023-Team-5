...
def _make_api_call(operation_name, **operation_options):
    return aws.make_api_call("elasticbeanstalk", operation_name, **operation_options)


def describe_configuration_options(**kwargs):
    LOG.debug("Inside describe_configuration_options api wrapper")
    result = _make_api_call("describe_configuration_options", **kwargs)
    return result
...
def list_application_load_balancers(platform, vpc=None):
    ...
    alb_list = next(
        (
            option["ValueOptions"]
            for option in describe_configuration_options_result["Options"]
            if option["Name"] == "SharedLoadBalancer"
        ),
        None,
    )
...
def delete_platform(arn):
    ...
    return _make_api_call(
        "delete_platform_version", PlatformArn=arn, DeleteResources=True
    )
...
def describe_platform_version(arn):
    ...
    return _make_api_call("describe_platform_version", PlatformArn=arn)[
        "PlatformDescription"
    ]


def _list_platform_branches(**kwargs):
    response = _make_api_call("list_platform_branches", **kwargs)
...
def _list_platform_versions(kwargs, nextToken=None):
    ...
    response = _make_api_call("list_platform_versions", **kwargs)
...
def create_application_version(
    ...
    if s3_bucket and s3_key:
        if build_configuration is None:
            kwargs["SourceBundle"] = {"S3Bucket": s3_bucket, "S3Key": s3_key}
        else:
    ...
    if build_configuration is not None:
        kwargs["BuildConfiguration"] = {
            "CodeBuildServiceRole": build_configuration.service_role,
            ...
            "TemplateSnippets": [
                {
                    ...
                    "elasticbeanstalk-env-resources-"
                    + region
...
def delete_application(app_name):
    ...
    result = _make_api_call("delete_application", ApplicationName=app_name)
...
def delete_application_and_envs(app_name):
    ...
    result = _make_api_call(
        "delete_application", ApplicationName=app_name, TerminateEnvByForce=True
    )
    return result["ResponseMetadata"]["RequestId"]
...
def describe_application(app_name):
    ...
    result = _make_api_call("describe_applications", ApplicationNames=[app_name])
...
def is_cname_available(cname):
    ...
    result = _make_api_call("check_dns_availability", CNAMEPrefix=cname)
...
def get_option_setting(option_settings, namespace, option):
    for setting in option_settings:
        if setting["Namespace"] == namespace and setting["OptionName"] == option:
            try:
...
def create_option_setting(namespace, option, value):
    return {"Namespace": namespace, "OptionName": option, "Value": value}
...
def get_application_versions(
    app_name, version_labels=None, max_records=None, next_token=None
):
    ...
    if next_token:
        ...
        kwargs["NextToken"] = next_token
    result = _make_api_call(
        "describe_application_versions", ApplicationName=app_name, **kwargs
    )
    return result


def application_version_exists(app_name, version_label):
    app_versions = get_application_versions(app_name, version_labels=[version_label])[
        "ApplicationVersions"
    ]
...
def get_environment(
    ...
    result = _make_api_call(
        "describe_environments", IncludeDeleted=include_deleted, **kwargs
    )
...
def get_environments(env_names=None):
    ...
    result = _make_api_call(
        "describe_environments", EnvironmentNames=env_names or [], IncludeDeleted=False
    )
...
def get_environment_resources(env_name):
    ...
    result = _make_api_call("describe_environment_resources", EnvironmentName=env_name)
    return result
...
def get_new_events(
    ...
    result = _make_api_call("describe_events", **kwargs)
...
def update_environment(
    ...
    if template_body:
        kwargs["TemplateSpecification"] = {
            "TemplateSource": {"SourceContents": template_body}
        }
    ...
    try:
        response = _make_api_call("update_environment", **kwargs)
    except aws.InvalidParameterValueError as e:
        if e.message == responses["env.invalidstate"].replace("{env-name}", env_name):
...
def abort_environment_update(env_name):
    ...
    result = _make_api_call("abort_environment_update", EnvironmentName=env_name)
    return result["ResponseMetadata"]["RequestId"]
...
def update_env_application_version(env_name, version_label, group_name):
    ...
    if group_name:
        ...
        response = _make_api_call(
            "update_environment", EnvironmentName=env_name, VersionLabel=version_label
        )
    return response["ResponseMetadata"]["RequestId"]
...
def request_environment_info(env_name, info_type):
    result = _make_api_call(
        "request_environment_info", EnvironmentName=env_name, InfoType=info_type
    )
    return result
...
def retrieve_environment_info(env_name, info_type):
    result = _make_api_call(
        "retrieve_environment_info", EnvironmentName=env_name, InfoType=info_type
    )
    return result
...
def create_configuration_template(app_name, env_name, template_name, description, tags):
    kwargs = {
        "TemplateName": template_name,
        "ApplicationName": app_name,
        "Description": description,
        "TemplateSpecification": {"TemplateSource": {"EnvironmentName": env_name}},
        "Tags": tags,
    }
    try:
        result = _make_api_call("create_configuration_template", **kwargs)
    except InvalidParameterValueError as e:
        if e.message == responses["cfg.nameexists"].replace("{name}", template_name):
...
def validate_template(app_name, template_name, platform=None):
    ...
    if platform:
        if PlatformVersion.is_valid_arn(platform):
            kwargs["TemplateSpecification"] = {
                "TemplateSource": {"PlatformArn": platform}
            }
        else:
            kwargs["TemplateSpecification"] = {
                "TemplateSource": {"SolutionStackName": platform}
            }
...
def rebuild_environment(env_id=None, env_name=None):
    ...
    result = _make_api_call("rebuild_environment", **kwargs)
...
def list_tags_for_resource(resource_arn):
    response = _make_api_call("list_tags_for_resource", ResourceArn=resource_arn)
