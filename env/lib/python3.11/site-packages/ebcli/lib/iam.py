...
def get_role(role_name):
    result = _make_api_call("get_role", RoleName=role_name)
    return result["Role"]
...
def create_instance_profile(profile_name, allow_recreate=True):
    ...
    """
    try:
        _make_api_call("create_instance_profile", InstanceProfileName=profile_name)
        return profile_name
...
def add_role_to_profile(profile, role, allow_recreate=True):
    ...
    try:
        _make_api_call(
            "add_role_to_instance_profile", InstanceProfileName=profile, RoleName=role
        )
    except AlreadyExistsError:
...
def create_role_with_policy(
    role_name, trust_document, policy_arns, allow_recreate=True
):
    """
...
def create_policy(policy_name, policy):
    result = _make_api_call(
        "create_policy", PolicyName=policy_name, PolicyDocument=policy
    )
    return result["Policy"]["Arn"]


def attach_role_policy(role_name, policy_arn):
    _make_api_call("attach_role_policy", RoleName=role_name, PolicyArn=policy_arn)
...
def create_role(role_name, document, allow_recreate=True):
    ...
    try:
        _make_api_call(
            "create_role", RoleName=role_name, AssumeRolePolicyDocument=document
        )
        return role_name
    except AlreadyExistsError:
        if allow_recreate:
...
def upload_server_certificate(cert_name, cert, private_key, chain=None):
    kwargs = dict(
        ServerCertificateName=cert_name, CertificateBody=cert, PrivateKey=private_key
    )
    if chain:
        kwargs["CertificateChain"] = chain

    result = _make_api_call("upload_server_certificate", **kwargs)
    return result["ServerCertificateMetadata"]


def get_managed_policy_document(arn):
    policy = _make_api_call("get_policy", PolicyArn=arn)
    policy_version = policy["Policy"]["DefaultVersionId"]
    details = _make_api_call(
        "get_policy_version", PolicyArn=arn, VersionId=policy_version
    )
