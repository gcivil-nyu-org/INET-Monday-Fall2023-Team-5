...
class ProjectRoot(object):
    ...
    def traverse(cls):
        ...
        if not os.path.isdir(beanstalk_directory):
            LOG.debug("beanstalk directory not found in " + cwd + "  -Going up a level")
...
def get_war_file_location():
    ...
    try:
        ...
        try:
            ...
            raise NotFoundError(
                "Can not find .war artifact in build"
                + os.path.sep
                + "libs"
                + os.path.sep
            )
...
def touch_config_folder(dir_path=None):
    if not os.path.isdir(
        os.path.join(dir_path, beanstalk_directory) if dir_path else beanstalk_directory
    ):
        os.makedirs(
...
def create_config_file(
    ...
    write_config_setting(
        "global", "default_platform", solution_stack, dir_path=dir_path
    )
    ...
    write_config_setting(
        "global", "platform_version", platform_version, dir_path=dir_path
    )
    write_config_setting(
        "global", "instance_profile", instance_profile, dir_path=dir_path
    )
...
def save_app_file(app):
    ...
    try:
        ...
        with codecs.open(file_name, "w", encoding="utf8") as f:
            f.write(safe_dump(app, default_flow_style=False, line_break=os.linesep))
...
def save_env_file(env):
    ...
    try:
        ...
        with codecs.open(file_name, "w", encoding="utf8") as f:
            f.write(safe_dump(env, default_flow_style=False, line_break=os.linesep))
                ...
                try:
                    ...
                    try:
                        ...
                        raise InvalidSyntaxError(
                            "The environment configuration contains invalid syntax. Make sure your input "
                            "matches one of the supported formats: JSON, YAML."
                        )
        else:
            raise NotFoundError(
                "The file you specified in this configuration path cannot be found: "
                + path
            )
...
def get_application_from_file(app_name):
    ...
    try:
        ...
        raise InvalidSyntaxError("The application file contains " "invalid syntax.")
...
def write_config_setting(
    section, key_name, value, dir_path=None, file=local_config_file
):
    cwd = os.getcwd()
    if dir_path:
        os.chdir(dir_path)
    try:
        ...
        if type(value) is dict:
            for key in value.keys():
                config.setdefault(section, {}).setdefault(key_name, {})[key] = value[
                    key
                ]
        else:
        ...
        with codecs.open(file, "w", encoding="utf8") as f:
            f.write(safe_dump(config, default_flow_style=False, line_break=os.linesep))
...
def write_json_dict(json_data, fullpath):
    ...
    data = json.dumps(json_data, sort_keys=True, indent=4, default=date_handler)
...
def get_build_configuration():
    ...
    try:
        ...
        if build_spec is None or buildspec_config_header not in build_spec.keys():
            ...
            io.log_warning(
                strings["codebuild.noheader"].replace(
...
def write_buildspec_config_header(key_name, value):
    write_config_setting(buildspec_config_header, key_name, value, file=buildspec_name)
...
def open_file_for_editing(file_location):
    ...
    try:
        ...
        io.log_error(prompts["fileopen.error1"].replace("{editor}", editor))
