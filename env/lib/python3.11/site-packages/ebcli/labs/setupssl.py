...
class SetupSSLController(AbstractBaseController):
    class Meta:
        ...
        arguments = AbstractBaseController.Meta.arguments + [
            ...
            (["--cert-chain"], dict(action="store", help="certificate chain file")),
            (
                ["--name"],
                dict(
                    action="store",
                    help="certificate name " "DEFAULT={environment-name}",
                ),
            ),
        ]
    ...
    def do_command(self):
        ...
        result = iam.upload_server_certificate(
            cert_name + ".crt", certfile, privatekey, chain=certchain
        )
        arn = result["Arn"]

        option_settings = [
            elasticbeanstalk.create_option_setting(
                namespaces.LOAD_BALANCER, option_names.LOAD_BALANCER_HTTP_PORT, "OFF"
            ),
            elasticbeanstalk.create_option_setting(
                namespaces.LOAD_BALANCER, option_names.LOAD_BALANCER_HTTPS_PORT, "443"
            ),
            elasticbeanstalk.create_option_setting(
                namespaces.LOAD_BALANCER, option_names.SSL_CERT_ID, arn
            ),
        ]
...
def generate_self_signed_cert(cert_name):
    ...
    if not fileoperations.program_is_installed("openssl"):
        raise CommandError(
            "This command requires openssl to be " "installed on the PATH"
        )
    ...
    if not os.path.isfile(sign_request_dir):
        ...
        subprocess.check_call(
            ["openssl", "req", "-new", "-key", privatekey_dir, "-out", sign_request_dir]
        )
        io.echo()
