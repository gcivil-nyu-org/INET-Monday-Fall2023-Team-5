...
class CreateEnvironmentRequest(object):
    def __init__(
        ...
        if not self.app_name:
            raise TypeError(
                self.__class__.__name__ + " requires key-word argument app_name"
            )
        if not self.env_name:
            raise TypeError(
                self.__class__.__name__ + " requires key-word argument env_name"
            )
    ...
    def add_option_setting(self, namespace, option_name, value, resource=None):
        setting = {"Namespace": namespace, "OptionName": option_name, "Value": value}
        if resource:
    ...
    def get_standard_kwargs(self):
        ...
        if self.scale:
            self.add_option_setting(
                namespaces.AUTOSCALING, option_names.MAX_SIZE, self.scale
            )
            self.add_option_setting(
                namespaces.AUTOSCALING, option_names.MIN_SIZE, self.scale
            )
    ...
    def compile_common_options(self):
        ...
        if self.service_role:
            self.add_option_setting(
                namespaces.ENVIRONMENT, option_names.SERVICE_ROLE, self.service_role
            )
        ...
        if self.instance_types:
            self.add_option_setting(
                namespaces.SPOT, option_names.INSTANCE_TYPES, self.instance_types
            )
        if self.single_instance:
            self.add_option_setting(
                namespaces.ENVIRONMENT, option_names.ENVIRONMENT_TYPE, "SingleInstance"
            )
        if self.key_name:
            self.add_option_setting(
                namespaces.LAUNCH_CONFIGURATION,
        ...
        if self.min_instances:
            self.add_option_setting(
                namespaces.AUTOSCALING, option_names.MIN_SIZE, self.min_instances
            )
        if self.max_instances:
            self.add_option_setting(
                namespaces.AUTOSCALING, option_names.MAX_SIZE, self.max_instances
            )
        if self.elb_type:
            self.add_option_setting(
                namespaces.ENVIRONMENT, option_names.LOAD_BALANCER_TYPE, self.elb_type
            )
    ...
    def add_client_defaults(self):
        ...
        if self.platform and self.platform.has_healthd_support:
            self.add_option_setting(
                namespaces.HEALTH_SYSTEM, option_names.SYSTEM_TYPE, "enhanced"
            )

        self.add_option_setting(namespaces.COMMAND, option_names.BATCH_SIZE, "30")
        self.add_option_setting(
            namespaces.COMMAND, option_names.BATCH_SIZE_TYPE, "Percentage"
        )
        if not self.tier or self.tier.name.lower() == "webserver":
            self.add_option_setting(
                namespaces.ELB_POLICIES, option_names.CONNECTION_DRAINING, "true"
            )
            self.add_option_setting(
                namespaces.LOAD_BALANCER, option_names.CROSS_ZONE, "true"
            )
    ...
    def compile_database_options(self):
        ...
        self.add_option_setting(
            namespace, option_names.DB_PASSWORD, self.database["password"]
        )
        self.add_option_setting(
            namespace, option_names.DB_USER, self.database["username"]
        )
        if self.database["instance"]:
            self.add_option_setting(
                namespace, option_names.DB_INSTANCE, self.database["instance"]
            )
        if self.database["size"]:
            self.add_option_setting(
                namespace, option_names.DB_STORAGE_SIZE, self.database["size"]
            )
        if self.database["engine"]:
            self.add_option_setting(
                namespace, option_names.DB_ENGINE, self.database["engine"]
            )
        if self.database["version"]:
            self.add_option_setting(
                namespace, option_names.DB_ENGINE_VERSION, self.database["version"]
            )
        self.add_option_setting(namespace, option_names.DB_DELETION_POLICY, "Snapshot")
    ...
    def compile_vpc_options(self):
        ...
        self.add_option_setting(namespace, option_names.VPC_ID, self.vpc["id"])
        if self.vpc["publicip"]:
            self.add_option_setting(
                namespace, option_names.PUBLIC_IP, self.vpc["publicip"]
            )
        if self.vpc["elbscheme"]:
            self.add_option_setting(
                namespace, option_names.ELB_SCHEME, self.vpc["elbscheme"]
            )
        if self.vpc["elbsubnets"]:
            self.add_option_setting(
                namespace, option_names.ELB_SUBNETS, self.vpc["elbsubnets"]
            )
        if self.vpc["ec2subnets"]:
            self.add_option_setting(
                namespace, option_names.SUBNETS, self.vpc["ec2subnets"]
            )
        ...
        if self.vpc["dbsubnets"]:
            self.add_option_setting(
                namespace, option_names.DB_SUBNETS, self.vpc["dbsubnets"]
            )
    ...
    def compile_spot_options(self):
        ...
        if self.on_demand_base_capacity:
            self.add_option_setting(
                namespace,
                option_names.ON_DEMAND_BASE_CAPACITY,
        ...
        if self.spot_max_price:
            self.add_option_setting(
                namespace, option_names.SPOT_MAX_PRICE, self.spot_max_price
            )
    ...
    def compile_shared_lb_options(self):
        ...
        if self.shared_lb:
            self.add_option_setting(
                namespaces.ENVIRONMENT, option_names.LOAD_BALANCER_IS_SHARED, "true"
            )
            self.add_option_setting(
        ...
        if self.shared_lb_port and self.shared_lb:
            ...
            self.add_option_setting(namespace, option_names.LISTENER_RULE, "default")
...
class CloneEnvironmentRequest(CreateEnvironmentRequest):
    def __init__(
        ...
        if not original_name:
            raise TypeError(
                self.__class__.__name__ + " requires key-word argument clone_name"
            )
        ...
        self.description = strings["env.clonedescription"].replace(
