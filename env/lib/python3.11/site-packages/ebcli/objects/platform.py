...
class PlatformVersion(object):
    ...
    def arn_to_platform(cls, arn):
        ...
        if not match:
            raise PlatformVersion.UnableToParseArnException(
                "Unable to parse arn '{}'".format(arn)
            )
    ...
    @classmethod
    def match_with_complete_arn(cls, platforms, input_platform_name):
        for platform in platforms:
            if platform == input_platform_name:
                return PlatformVersion(platform)

    @classmethod
    def match_with_platform_name(cls, custom_platforms, input_platform_name):
    ...
    def __init__(
        ...
        (
            account_id,
            arn_platform_name,
            arn_platform_version,
        ) = PlatformVersion.arn_to_platform(self.platform_arn)
    ...
    def has_healthd_group_version_2_support(self):
        ...
        return utils.parse_version(self.platform_version) >= utils.parse_version(
            "2.0.10"
        )
    ...
    def is_recommended(self):
        return (
            ...
            == platform_version_lifecycle_states.RECOMMENDED
        )
    ...
    def hydrate(self, describe_platform_version):
        ...
        if not self._is_hydrated():
            platform_version_description = describe_platform_version(self.platform_arn)
...
class PlatformBranch(object):
    ...
    def __lt__(self, other):
        ...
        self_comp_value = (
            self.platform_name,
            lifecycle_sort_values.get(
                self.lifecycle_state, default_lifecycle_sort_value
            ),
            self.branch_name,
        )
        other_comp_value = (
            other.platform_name,
            lifecycle_sort_values.get(
                other.lifecycle_state, default_lifecycle_sort_value
            ),
            other.branch_name,
        )
        return self_comp_value < other_comp_value
    ...
    def hydrate(self, get_platform_branch_by_name):
        ...
        """
        if not self._is_hydrated():
            platform_branch_summary = get_platform_branch_by_name(self.branch_name)
