...
class EnvironmentSettings(ConversionConfiguration):
    def collect_changes(self, usr_model):
        ...
        for setting in option_settings:
            ...
            try:
                ...
                d = _get_option_setting_dict(namespace, option, None, resource_name)
                remove.append(d)

        # Now we look for added options:
        for namespace, options in six.iteritems(usr_options):
            if options:
                namespace, resource_name = _get_namespace_and_resource_name(namespace)
    ...
    def remove_unwanted_settings(self):
        ...
        self.api_model["OptionSettings"] = [
            setting
            for setting in option_settings
            if setting["Namespace"] != ENVIRONMENT_VAR_NAMESPACE
            and setting["Namespace"] != CLOUDFORMATION_TEMPLATE
            and not (
                setting["Namespace"] == DATABASE_NAMESPACE
                and (
                    setting["OptionName"] == "DBEngineVersion"
                    or setting["OptionName"] == "DBUser"
                    or setting["OptionName"] == "DBEngine"
                    or setting["OptionName"] == "DBPassword"
                )
            )
        ]
    ...
    def ignore_default_resource_names(self):
        ...
        for setting in option_settings:
            if "ResourceName" in setting and setting["ResourceName"] in [
                ...
                "AWSEBRDSDatabase",
                "AWSEBSecurityGroup",
                "AWSEBV2LoadBalancer" "AWSEBV2LoadBalancerListener",
    ...
    def convert_usr_model_to_api(settings):
        ...
        for resource_namespace, options in settings.items():
            namespace, resource_name = _get_namespace_and_resource_name(
                resource_namespace
            )
            if isinstance(options, dict):
                for option_name, value in options.items():
                    changes.append(
                        _get_option_setting_dict(
                            namespace, option_name, value, resource_name
                        )
                    )
            else:
                for option_name in options:
                    changes.append(
                        _get_option_setting_dict(
                            namespace, option_name, None, resource_name
                        )
                    )
        return changes
