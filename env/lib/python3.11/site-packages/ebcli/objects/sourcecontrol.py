...
class Git(SourceControl):
    ...
    def get_version_label(self):
        ...
        stdout, stderr, exitcode = self._run_cmd(
            ["git", "describe", "--always", "--abbrev=4"]
        )

        version_label = "app-{}-{:%y%m%d_%H%M%S%f}".format(
    ...
    def get_current_repository(self):
        ...
        get_remote_name_command = [
            ...
            "config",
            "--get",
            "branch.{0}.remote".format(current_branch),
        ]
        LOG.debug(
        ...
        stdout, stderr, exitcode = self._run_cmd(
            get_remote_name_command, handle_exitcode=False
        )
    ...
    def get_current_branch(self):
        ...
        LOG.debug(
            "Getting current branch name by performing `{0}`".format(
                " ".join(revparse_command)
            )
        )

        stdout, stderr, exitcode = self._run_cmd(
            revparse_command, handle_exitcode=False
        )
    ...
    def get_current_commit(self):
        ...
        LOG.debug(
            "Getting current commit by performing `{0}`".format(
                " ".join(latest_commit_command)
            )
        )

        stdout, stderr, exitcode = self._run_cmd(
            latest_commit_command, handle_exitcode=False
        )
        if exitcode != 0:
    ...
    def do_zip_submodule(
        self, main_location, sub_location, staged=False, submodule_dir=None
    ):
        if staged:
    ...
    def do_zip(self, location, staged=False):
        ...
        try:
            ...
            stdout, stderr, exitcode = self._run_cmd(
                ["git", "archive", "-v", "--format=zip", "-o", location, commit_id]
            )
            io.log_info("git archive output: {0}".format(stderr))

            project_root = os.getcwd()

            must_zip_submodules = fileoperations.get_config_setting(
                "global", "include_git_submodules"
            )
            ...
            if must_zip_submodules:
                ...
                for index, line in enumerate(stdout.splitlines()):
                    ...
                    self.do_zip_submodule(
                        location,
                        "{0}_{1}".format(location, str(index)),
                        staged=staged,
    ...
    def get_message(self):
        stdout, stderr, exitcode = self._run_cmd(["git", "log", "--oneline", "-1"])
    ...
    def push_codecommit_code(self):
        ...
        stdout, stderr, exitcode = self._run_cmd(
            ["git", "push", self.get_current_repository(), self.get_current_branch()]
        )
    ...
    def setup_codecommit_remote_repo(self, remote_url):
        ...
        stdout, stderr, exitcode = self._run_cmd(
            remote_add_command, handle_exitcode=False
        )
        ...
        if exitcode != 0:
            if exitcode == 128:
                ...
                LOG.debug(
                    "Error setting up git config for CodeCommit: {0}".format(stderr)
                )
                return
        else:
    ...
    def checkout_branch(self, branch_name, create_branch=False):
        stdout, stderr, exitcode = self._run_cmd(
            ["git", "checkout", branch_name], handle_exitcode=False
        )
    ...
    def create_initial_commit(self):
        ...
        if exitcode != 0:
            LOG.debug(
                "git was not able to initialize an empty commit: {0}".format(stderr)
            )
    ...
    def fetch_remote_branches(self, remote_name):
        ...
        LOG.debug(
            "Fetching remote branches using remote name: {0}".format(
                " ".join(fetch_command)
            )
        )
    ...
    def verify_url_is_a_codecommit_url(self, remote_url):
        ...
        if not codecommit_url_regex.search(remote_url):
            ...
            raise NoSourceControlError(
                "Could not connect to repository located at {}".format(remote_url)
            )
