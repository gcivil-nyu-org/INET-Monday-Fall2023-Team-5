...
class Environment(object):
    ...
    @classmethod
    def json_to_environment_object(cls, environment_json, want_solution_stack=False):
    ...
    @classmethod
    def json_to_environment_objects_array(cls, json, want_solution_stack=False):
        environments = []
        for environment_json in json:
            environments.append(
                Environment.json_to_environment_object(
                    environment_json, want_solution_stack
                )
            )
    ...
    def print_env_details(
        ...
        self.print_env_links(
            echo_method, get_environments_callback, get_environment_resources_callback
        )
    ...
    def print_env_links(
        self, echo_method, get_environments_callback, get_environment_resources_callback
    ):
        if self.environment_links and len(self.environment_links) > 0:
            ...
            for link in self.environment_links:
                link_data = dict(
                    link_name=link["LinkName"], env_name=link["EnvironmentName"]
                )
            ...
            for link in links.values():
                ...
                echo_method("      {}: {}".format(link["link_name"], link["value"]))

    def get_worker_sqs_url(self, get_environment_resources_callback):
        resources = get_environment_resources_callback(self.name)[
    ...
    def __region_from_environment_arn(self):
        environment_arn_regex = re.compile(
            r"arn:aws.*:elasticbeanstalk:(.*):\d+:environment/.*/.*$"
        )
