...
def validate_restore(env_id):
    ...
    env = elasticbeanstalk.get_environment(
        env_id=env_id, include_deleted=True, deleted_back_to=get_date_cutoff()
    )

    if env.status != "Terminated":
        raise InvalidParameterValueError(
            "Environment {0} ({1}) is currently {2}, must "
            'be "Terminated" to restore'.format(env.name, env.id, env.status)
        )
...
def restore(env_id, timeout=None):
    ...
    commonops.wait_for_success_events(
        request_id, timeout_in_minutes=timeout, can_abort=True
    )
...
def get_restorable_envs(app_name):
    ...
    app_envs = elasticbeanstalk.get_raw_app_environments(
        app_name, include_deleted=True, deleted_back_to=date_cutoff
    )
    for env in app_envs:
        if env["Status"] == "Terminated":
    ...
    environments = sorted(
        environments, key=operator.itemgetter("DateUpdated"), reverse=True
    )
    return environments
...
def display_environments(environments, timeout_in_minutes=5):
    ...
    try:
        ...
        if hasattr(screen, "request_id") and screen.request_id:
            ...
            commonops.wait_for_success_events(
                screen.request_id, timeout_in_minutes, can_abort=True
            )
    finally:
