...
def create_app_version_without_deployment(
    app_name,
    label=None,
    staged=False,
    process=False,
    ...
    if build_config is not None:
        ...
        commonops.wait_for_processed_app_versions(
            app_name, [app_version_label], timeout=timeout
        )
...
def delete_app_version_label(app_name, version_label):
    if version_label:
        ...
        if not any(
            version_label == app_version["VersionLabel"] for app_version in app_versions
        ):
            raise ValidationError(
                strings["appversion.delete.notfound"].format(app_name, version_label)
            )
        ...
        used_envs = [
            version[1] for version in versions_in_use if version[0] == version_label
        ]
...
def _create_version_table(screen):
    ...
    screen.add_table(
        Table(
            VersionScreen.APP_VERSIONS_TABLE_NAME,
            columns=[
