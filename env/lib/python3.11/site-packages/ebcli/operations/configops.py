...
def display_environment_configuration(app_name, env_name, output_format="yaml"):
    api_model = elasticbeanstalk.describe_configuration_settings(app_name, env_name)
...
def modify_environment_configuration(env_name, usr_modification, nohang, timeout=None):
    if usr_modification.startswith("file:/"):
        ...
        usr_modification = fileoperations.get_environment_from_file(
            env_name, path=file_path
        )
    else:
        try:
    ...
    if "OptionSettings" in usr_modification:
        changes = EnvironmentSettings.convert_usr_model_to_api(
            usr_modification["OptionSettings"]
        )
    remove = []
    if "OptionsToRemove" in usr_modification:
        remove = EnvironmentSettings.convert_usr_model_to_api(
            usr_modification["OptionsToRemove"]
        )
    platform_arn = None
