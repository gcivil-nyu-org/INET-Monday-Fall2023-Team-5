...
def get_codebuild_image_from_platform(platform):
    ...
    if not isinstance(beanstalk_images, list) or not all(
        isinstance(i, dict) for i in beanstalk_images
    ):
        LOG.error(
            "Unexpected format for beanstalk_images. Expected a list of dictionaries."
        )
        return []
    ...
    LOG.debug(
        "Searching for images for platform '{0}'. Found: {1}".format(
