...
def find_solution_stack_from_string(solution_string, find_newer=False):
    ...
    if PlatformVersion.is_eb_managed_platform_arn(solution_string):
        ...
        if find_newer:
            match = platform_version_ops.get_latest_custom_platform_version(
                solution_string
            )
        else:
            match = platform_version_ops.find_custom_platform_version_from_string(
                solution_string
            )
    ...
    if not match:
        ...
        match = SolutionStack.match_with_complete_solution_string(
            available_solution_stacks, solution_string
        )
        if match and find_newer:
            ...
            match = SolutionStack.match_with_solution_string_language_name(
                available_solution_stacks, language_name
            )
...
def resolve_language_version(chosen_language_name, solution_stacks):
    ...
    matching_language_versions = (
        SolutionStack.group_solution_stacks_by_platform_shorthand(
            solution_stacks, language_name=chosen_language_name
        )
    )
