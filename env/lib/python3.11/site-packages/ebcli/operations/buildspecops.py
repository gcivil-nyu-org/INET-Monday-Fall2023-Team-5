...
def stream_build_configuration_app_version_creation(
    app_name, app_version_label, build_spec
):
    # Get the CloudWatch logs link
    successfully_generated = wait_for_app_version_attribute(
        app_name, [app_version_label], timeout=1
    )
    app_version_response = elasticbeanstalk.get_application_versions(
        app_name, version_labels=[app_version_label]
    )["ApplicationVersions"]

    build_response = (
        codebuild.batch_get_builds([app_version_response[0]["BuildArn"]])
    ...
    if build_response is not None and "logs" in build_response["builds"][0]:
        ...
        io.echo(
            ...
            "operation may take upto '{0}' minutes to complete.".format(
                codebuild_timeout
            )
        )
    else:
    ...
    try:
        ...
        timeout_error_message = " ".join(
            [
                "The CodeBuild build timed out after {} minute(s).".format(
                    codebuild_timeout
                ),
        ...
        LOG.debug(
            ...
            "deleting the created application version as it is useless now.".format(
                app_version_label
            )
        )
    ...
    while versions_to_check:
        ...
        for version in app_versions:
            if version.get("BuildArn"):
                ...
                io.echo(
                    strings["appversion.attribute.success"].format(
