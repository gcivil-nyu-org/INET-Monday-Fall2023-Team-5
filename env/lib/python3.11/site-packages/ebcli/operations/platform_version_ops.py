...
class PackerStreamMessage(object):
    ...
    def format(self):
        ...
        if other_packer_message:
            ...
            formatted_other_message = "{}:{}".format(
                other_packer_message_target, other_packer_message
            )
    ...
    def other_packer_message_target(self):
        return self.__return_match(
            PackerRegExpressions.PACKER_OTHER_MESSAGE_TARGET_REGEX
        )
...
def delete_platform_version(platform_version, force=False):
    ...
    try:
        environments = [
            env
            for env in elasticbeanstalk.get_environments()
    ...
    if len(environments) > 0:
        ...
        raise ValidationError(
            strings["platformdeletevalidation.error"].format(
                platform_name,
                platform_version,
    ...
    commonops.wait_for_success_events(
        request_id, timeout_in_minutes=timeout, platform_arn=arn
    )
...
def find_custom_platform_version_from_string(solution_string):
    ...
    for custom_platform_matcher in [
        ...
        matched_custom_platform = custom_platform_matcher(
            available_custom_platforms, solution_string
        )
...
def get_platforms(
    platform_name=None, ignored_states=None, owner=None, platform_version=None
):
...
def get_platform_arn(platform_name, platform_version, owner=None):
    platform = describe_custom_platform_version(
        platform_name=platform_name, platform_version=platform_version, owner=owner
    )
...
def get_preferred_platform_version_for_branch(branch_name):
    ...
    matched_versions = list(
        sorted(matched_versions, key=lambda x: x.sortable_version, reverse=True)
    )
    ...
    if len(recommended_versions) > 0:
        ...
        raise NotFoundError(alerts["platform.invalidstring"].format(branch_name))
...
def list_custom_platform_versions(
    ...
    return list_platform_versions(
        filters, platform_name, platform_version, show_status, status
    )
...
def list_platform_versions(
    filters, platform_name=None, platform_version=None, show_status=False, status=None
):
    if platform_name:
        filters.append(api_filters.PlatformNameFilter(values=[platform_name]).json())
    ...
    if status:
        filters.append(api_filters.PlatformStatusFilter(values=[status]).json())
...
def version_to_arn(platform_version):
    ...
    if PlatformRegExpressions.VALID_PLATFORM_VERSION_FORMAT.match(platform_version):
        arn = get_platform_arn(
            platform_name, platform_version, owner=Constants.OWNED_BY_SELF
        )
        ...
        match = PlatformRegExpressions.VALID_PLATFORM_SHORT_FORMAT.match(
            platform_version
        )
...
def _create_app_version_zip_if_not_present_on_s3(
    ...
    s3_bucket, s3_key = commonops.get_app_version_s3_location(
        platform_name, version_label
    )
    file_name, file_path = None, None
    if s3_bucket is None and s3_key is None:
        file_name, file_path = commonops._zip_up_project(
...
def _resolve_s3_bucket_and_key(platform_name, version_label, source_control, staged):
...
def _resolve_version_number(
    ...
    version = get_latest_platform_version(
        platform_name=platform_name, owner=Constants.OWNED_BY_SELF, ignored_states=[]
    )
...
def __formatted_platform_descriptions(platforms_list, show_status):
    ...
    for index, platform_tuple in enumerate(platform_tuples):
        if show_status:
            formatted_platform_description = (
                "{platform_arn}  Status: {platform_status}".format(
