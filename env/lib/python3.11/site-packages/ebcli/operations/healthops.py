...
from ebcli.display.traditional import (
    TraditionalHealthScreen,
    TraditionalHealthDataPoller,
)
...
def display_interactive_health(app_name, env_name, refresh, mono, default_view):
    env = elasticbeanstalk.describe_configuration_settings(app_name, env_name)
    health_type = elasticbeanstalk.get_option_setting(
        env.get("OptionSettings"), namespaces.HEALTH_SYSTEM, option_names.SYSTEM_TYPE
    )
    ...
    if health_type == option_values.SYSTEM_TYPE__ENHANCED:
        ...
        platform = (
            ...
            if env.get("PlatformArn")
            else SolutionStack(env["SolutionStackName"])
        )
    ...
    try:
        screen.start_screen(poller, env, refresh, mono=mono, default_table=default_view)
    finally:
