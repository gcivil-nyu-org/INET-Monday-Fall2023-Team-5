                ...
                if len(events) > 0:
                    ...
                    if stream_events:
                        streamer.stream_event(
                            get_event_string(event, long_format=True),
...
def filter_events(events, version_label=None, request_id=None, env_name=None):
    ...
    for event in events:
        if (
            version_label
            and event.version_label
            and (event.version_label != version_label)
        ):
            continue
...
def wait_for_compose_events(
    ...
    try:
        while not _timeout_reached(start, timediff):
            ...
            compose_events = elasticbeanstalk.get_new_events(
                app_name=app_name,
                env_name=None,
                request_id=request_id,
...
def _raise_if_error_event(message):
    ...
    if message.startswith(
        responses["event.platform_ami_region_service_region_mismatch"]
    ):
        raise ServiceError(message)
    if (
    ...
    if message.startswith(responses["appversion.finished"]) and message.endswith(
        "FAILED."
    ):
        raise ServiceError(message)
...
def get_event_string(event, long_format=False):
    ...
    if long_format:
        return "{0} {1} {2}".format(
            date.strftime("%Y-%m-%d %H:%M:%S").ljust(22), severity.ljust(7), message
        )
    else:
...
def get_compose_event_string(event, long_format=False):
    ...
    if long_format:
        ...
        return "{0} - {1}: {2}".format(app_name, severity, message)
...
def get_env_event_string(event, long_format=False):
    ...
    if long_format:
        ...
        return "{0} - {1}: {2}".format(environment.rjust(40), severity, message)
    ...
    if app_version:
        ...
        io.log_info(
            "Application Version '{0}' exists. Source from S3: {1}/{2}.".format(
                version_label, s3_bucket, s3_key
            )
        )
...
def create_app(app_name, default_env=None, tags=[]):
    try:
        ...
        elasticbeanstalk.create_application(app_name, strings["app.description"], tags)

        set_environment_for_current_branch(None)
        set_group_suffix_for_current_branch(None)

        io.echo("Application", app_name, "has been created.")
        return None, None
...
def pull_down_app_info(app_name, default_env=None):
    ...
    if len(envs) == 0:
        ...
        env = envs[0]
        io.log_info('Setting only environment "' + env.name + '" as default')
    elif len(envs) > 1:
        if default_env:
            if default_env == "/ni":
                env = envs[0]
    ...
    keyname = elasticbeanstalk.get_specific_configuration_for_env(
        app_name, env.name, "aws:autoscaling:launchconfiguration", "EC2KeyName"
    )
    if keyname is None:
        keyname = -1
...
def open_webpage_in_browser(url, ssl=False):
    ...
    if utils.is_ssh() or platform.system().startswith("Win"):
        ...
        p = Popen(
            ["{python} -m webbrowser '{url}'".format(python=sys.executable, url=url)],
            stderr=PIPE,
            stdout=PIPE,
            shell=True,
        )
...
def create_dummy_app_version(app_name):
    ...
    return _create_application_version(
        app_name, version_label, None, None, None, warning=False
    )


def create_app_version(
    app_name, process=False, label=None, message=None, staged=False, build_config=None
):
    cwd = os.getcwd()
...
def create_codecommit_app_version(
    app_name, process=False, label=None, message=None, build_config=None
):
    fileoperations.ProjectRoot.traverse()
...
def create_app_version_from_source(
    app_name, source, process=False, label=None, message=None, build_config=None
):
    cwd = os.getcwd()
    fileoperations.ProjectRoot.traverse()
    try:
    ...
    if source_location == "codecommit":
        try:
            ...
            io.log_error(
                ...
                "because of this error: {2}".format(branch, repository, ex.code)
            )
            raise ex
...
def _create_application_version(
    ...
    while True:
        try:
            ...
            if e.message.startswith("Application Version ") and e.message.endswith(
                " already exists."
            ):
                if warning:
...
def _zip_up_project(version_label, source_control, staged=False):
    ...
    if not fileoperations.file_exists(file_path):
        io.echo(strings["appversion.create"].replace("{version}", version_label))
...
def update_environment(
    ...
    wait_for_success_events(request_id, timeout_in_minutes=timeout, can_abort=True)
...
def write_setting_to_current_branch(keyname, value):
    ...
    fileoperations.write_config_setting(
        "branch-defaults", branch_name, {keyname: value}
    )
...
def _get_public_ssh_key(keypair_name):
    ...
    if os.path.exists(key_file):
        ...
        raise NotSupportedError(
            strings["ssh.filenotfound"].replace("{key-name}", keypair_name)
        )

    try:
        stdout, stderr, returncode = exec_cmd(["ssh-keygen", "-y", "-f", file_name])
        if returncode != 0:
            raise CommandError(
                "An error occurred while trying " "to get ssh public key"
            )
        key_material = stdout
        return key_material
    except OSError:
...
def wait_for_processed_app_versions(app_name, version_labels, timeout=5):
    ...
    while not all(
        [(processed[version] or failed[version]) for version in versions_to_check]
    ):
        ...
        for v in app_versions:
            if v["Status"] == "PROCESSED":
                ...
                io.echo(
                    "Finished processing application version {}".format(
                        v["VersionLabel"]
                    )
                )
                ...
                io.log_error(
                    strings["appversion.processfailed"].replace(
                        "{app_version}", v["VersionLabel"]
                    )
                )
                versions_to_check.remove(v["VersionLabel"])
...
def create_instance_profile(
    ...
    try:
        ...
        io.log_warning(
            strings["platformcreateiamdescribeerror.info"].format(
...
def credentials_are_valid():
    try:
        ...
        io.log_error(
            "The current user does not have the correct permissions. "
...
def setup_credentials(access_id=None, secret_key=None):
    ...
    if access_id is None:
        access_id = io.prompt("aws-access-id", default="ENTER_AWS_ACCESS_ID_HERE")
...
def get_region(
    region_argument, interactive, force_non_interactive=False, platform=None
):
...
def check_credentials(
    profile, given_profile, given_region, interactive, force_non_interactive
):
    try:
...
def raise_if_inside_application_workspace():
    ...
    if workspace_type and workspace_type == Constants.WorkSpaceTypes.APPLICATION:
        raise EnvironmentError(
            strings["platforminit.application_workspace_already_initialized"]
        )
...
def set_up_credentials(
    given_profile, given_region, interactive, force_non_interactive=False
):
    if given_profile:
...
def set_region_for_application(
    interactive, region, force_non_interactive, platform=None
):
