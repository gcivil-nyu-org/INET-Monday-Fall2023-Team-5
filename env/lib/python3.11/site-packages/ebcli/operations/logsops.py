...
def beanstalk_log_group_builder(env_name, log_group_name=None):
    ...
    if not log_group_name.startswith(
        cloudwatch_log_group_prefix_for_environment(env_name)
    ):
...
def cloudwatch_log_group_for_environment_health_streaming(env_name):
    ...
    """
    return "{0}/{1}".format(
        cloudwatch_log_group_prefix_for_environment(env_name), "environment-health.log"
    )
...
def deployment_logs_log_group_name(env_name):
    ...
    # will be no more new Amazon Linux 1 platforms so once we figured out a pattern, we would solve the issues forever.
        ...
        "Amazon Linux 2/" in environment.platform.name
        or "Amazon Linux 2023/" in environment.platform.name
    ):
    ...
    return (
        cloudwatch_log_group_prefix_for_environment(env_name) + "/" + log_group_suffix
    )
...
def disable_cloudwatch_logs(app_name, env_name, cloudwatch_log_source):
    ...
    cloudwatch_log_source = (
        cloudwatch_log_source
        or logs_operations_constants.LOG_SOURCES.INSTANCE_LOG_SOURCE
    )
    ...
    if cloudwatch_log_source in [
        ...
        if instance_log_streaming_enabled(
    ...
    if cloudwatch_log_source in [
        ...
        if environment_health_streaming_enabled(
            app_name, env_name, config_settings=configuration_settings
        ):
            io.echo(strings["cloudwatch_environment_health_log_streaming.disable"])
            option_settings.append(
                _environment_health_log_streaming_option_setting(disable=True)
            )
        else:
            io.echo(
                strings["cloudwatch_environment_health_log_streaming.already_disabled"]
            )

    if option_settings:
        commonops.update_environment(
            env_name, changes=option_settings, nohang=False, timeout=timeout
        )
...
def enable_cloudwatch_logs(app_name, env_name, cloudwatch_log_source):
    ...
    if cloudwatch_log_source in [
        ...
        if not environment_health_streaming_enabled(
            ...
            io.echo(
                strings["cloudwatch_environment_health_log_streaming.already_enabled"]
            )
...
def environment_health_streaming_enabled(app_name, env_name, config_settings=None):
    ...
    config_settings = (
        config_settings
        or elasticbeanstalk.describe_configuration_settings(app_name, env_name)
    )
...
def get_cloudwatch_log_stream_events(log_group_name, stream_name, num_log_events=None):
    ...
    try:
        response = cloudwatch.get_log_events(
            log_group_name, stream_name, limit=num_log_events
        )

        for event in response.get("events", []):
            message = event.get("message")
            full_log.append(
                "[{stream_name}] {message}".format(
                    stream_name=stream_name, message=message
                )
            )
...
def normalize_log_group_name(env_name, log_group=None, cloudwatch_log_source=None):
    ...
    if (
        ...
        log_group = beanstalk_log_group_builder(
            env_name, cloudwatch_log_group_for_environment_health_streaming(env_name)
        )
    else:
        raise InvalidOptionsError(
            strings[
...
def raise_if_instance_log_streaming_is_not_enabled(app_name, env_name):
    ...
    if not instance_log_streaming_enabled(app_name, env_name):
        raise InvalidOptionsError(
            strings["logs.instance_log_streaming_disabled"].format(env_name)
        )
...
def raise_if_environment_health_log_streaming_is_not_enabled(app_name, env_name):
    ...
    if not environment_health_streaming_enabled(app_name, env_name):
        raise InvalidOptionsError(
            strings["logs.environment_health_log_streaming_disabled"].format(env_name)
        )
...
def retrieve_cloudwatch_instance_logs(
    log_group, info_type, do_zip=False, specific_log_stream=None
):
    """
    ...
    retrieve_cloudwatch_logs(
        log_group, info_type, do_zip, specific_log_stream=specific_log_stream
    )


def retrieve_cloudwatch_environment_health_logs(log_group, info_type, do_zip=False):
    """
...
def retrieve_cloudwatch_logs(
    ...
    log_streams = cloudwatch.get_all_stream_names(
        log_group_name=log_group, log_stream_name_prefix=specific_log_stream
    )
...
def stream_environment_health_logs_from_cloudwatch(
    sleep_time=10, log_group=None, specific_log_stream=None
):
    """
    ...
    for log_stream_name in other_log_stream_names:
        _create_log_stream_for_log_group(
            log_group, log_stream_name, streamer, sleep_time, start_time
        )
        _delay_subsequent_stream_creation()

    while True:
        _create_log_stream_for_log_group(
            log_group, latest_log_stream_name, streamer, sleep_time, start_time
        )
...
def stream_instance_logs_from_cloudwatch(
    ...
    while True:
        log_group_names = set(
            cloudwatch_log_stream_names(log_group, specific_log_stream)
        )

        for log_group_name in log_group_names:
            _create_log_stream_for_log_group(
                log_group, log_group_name, streamer, sleep_time, start_time
            )
            _delay_subsequent_stream_creation()
...
def stream_logs_in_terminal(log_group, log_streams):
    ...
    for log_stream in log_streams:
        tail_logs = get_cloudwatch_log_stream_events(
            log_group, log_stream, num_log_events=TAIL_LOG_SIZE
        )
        all_logs += (
            "{linesep}{linesep}============= "
            "{log_stream} - {log_group} =============="
            "{linesep}{linesep}".format(
                log_stream=str(log_stream), log_group=log_group, linesep=os.linesep
            )
        )
        all_logs += tail_logs
...
def stream_platform_logs(
    platform_name, version, streamer=None, sleep_time=4, log_name=None, formatter=None
):
    """
...
def stream_single_stream(
    ...
    get_cloudwatch_messages(
        log_group_name, stream_name, formatter, None, start_time, messages_handler
    )
...
def _attempt_update_symlink_to_latest_logs_retrieved(logs_location):
    ...
    latest_symlink_location = os.path.join(os.path.dirname(logs_location), "latest")
...
def _create_log_stream_for_log_group(
    log_group, stream_name, streamer, sleep_time, start_time=None
):
...
def _download_logs_for_all_instances(instance_id_list, logs_location):
    for instance_id, url in iteritems(instance_id_list):
        zip_location = utils.save_file_from_url(
            url, logs_location, instance_id + ".zip"
        )
...
def _echo_link_to_cloudwatch_console(env_name):
    ...
    io.echo(
        cw_link_regionalized.replace("{region}", region).replace("{env_name}", env_name)
    )
...
def _get_cloudwatch_messages(
    log_group_name, stream_name, formatter=None, next_token=None, start_time=None
):
    messages = []
    response = None
    ...
    try:
        response = cloudwatch.get_log_events(
            log_group_name, stream_name, next_token=next_token, start_time=start_time
        )
...
def _raise_if_environment_is_not_using_enhanced_health(configuration_settings):
    ...
    health_type = elasticbeanstalk.get_option_setting(
        option_settings, namespaces.HEALTH_SYSTEM, option_names.SYSTEM_TYPE
    )
...
def _updated_instance_id_list(instance_id_list, instance_id):
    try:
        return {instance_id: instance_id_list[instance_id]}
    except KeyError:
