...
class ArgumentSyntaxValidator(object):
    ...
    def validate_key_value_pair(cls, tag):
        ...
        if len(value) > 255:
            raise cls.InvalidTagValueError(
                (linesep * 2).join(
                    [strings["tags.tag_value_max_length_exceeded"], value]
                )
            )
        ...
        if not cls.__tag_component_regex_search(value_regex_matcher, value):
            raise cls.InvalidTagValueError(
                strings["tags.invalid_tag_value"].format(value)
            )
    ...
    def validate_key(cls, key):
        ...
        if len(key) > 127:
            raise cls.InvalidTagKeyError(
                (linesep * 2).join([strings["tags.tag_key_max_length_exceeded"], key])
            )
    ...
    def __tag_component_regex_matcher(cls, value):
        if sys.version_info < (3, 0):
            return re.compile(
                r"^[\w\s.:\\/+=@-]{" + str(len(value.decode("utf-8"))) + "}", re.UNICODE
            )
        else:
            return re.compile(r"^[\w\s.:\\/+=@-]{" + str(len(value)) + "}")
...
class TagListValidator(object):
    ...
    def __init__(self, taglist, key):
        ...
        self.additions = [
            addition
            for addition in taglist.additions
    ...
    def validate_key_for_addition_is_unique(self):
        ...
        self.__raise_if_key_present_in_list(
            list_of_keys_of(self.additions), "tags.duplicate_key_in_add_list"
        )
    ...
    def validate_key_for_deletion_is_unique(self):
        ...
        """
        self.__raise_if_key_present_in_list(
            self.deletions, "tags.duplicate_key_in_delete_list"
        )
        self.__raise_if_key_present_in_list(
            list_of_keys_of(self.updates), "tags.duplicate_key_in_update_list"
        )
...
class TagList(object):
    ...
    def populate_add_list(self, addition_string):
        ...
        for tag in tags_to_add:
            ...
            self.additions.append({"Key": key, "Value": value})
    ...
    def populate_update_list(self, update_string):
        ...
        for tag in tags_to_update:
            ...
            self.updates.append({"Key": key, "Value": value})
