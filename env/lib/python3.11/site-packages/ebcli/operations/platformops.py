...
def detect_platform_family(families_set):
    ...
    if detected_platform_family and detected_platform_family in families_set:
        io.echo()
        io.echo(prompts["platform.validate"].format(platform=detected_platform_family))
...
def get_environment_platform(app_name, env_name, want_solution_stack=False):
    env = elasticbeanstalk.get_environment(
        app_name=app_name, env_name=env_name, want_solution_stack=want_solution_stack
    )
    return env.platform
...
def get_platform_name_and_version_interactive():
    ...
    if len(platform_list) > 0:
        ...
        platform_name = utils.prompt_for_item_in_list(
            platform_list, default=default_option
        )
...
def get_version_status(version):
    ...
    io.echo(
        "Programming Language Version: ", language_version
    ) if language_version else None
...
def generate_version_to_arn_mappings(custom_platforms, specified_platform_name):
    ...
    for custom_platform in custom_platforms:
        ...
        if custom_platform_name == specified_platform_name:
            version_to_arn_mappings[
                PlatformVersion.get_platform_version(custom_platform)
            ] = custom_platform
...
def get_platform_for_platform_string(platform_string):
    ...
    if platform_branch_ops.is_platform_branch_name(platform_string):
        return platform_version_ops.get_preferred_platform_version_for_branch(
            platform_string
        )
...
def prompt_customer_for_custom_platform_version(version_to_arn_mappings):
    ...
    chosen_custom_platform_version = utils.prompt_for_item_in_list(
        custom_platform_versions_to_display
    )
...
def prompt_for_platform_branch(family):
    ...
    branch_display_names = [
        _generate_platform_branch_prompt_text(branch) for branch in branches
    ]
...
def prompt_for_platform():
    ...
    platform_family = prompt_for_platform_family(
        include_custom=enable_custom_platform_prompt
    )
...
def resolve_custom_platform_version(custom_platforms, selected_platform_name):
    version_to_arn_mappings = generate_version_to_arn_mappings(
        custom_platforms, selected_platform_name
    )
...
def set_workspace_to_latest():
    ...
    version = platform_version_ops.get_latest_platform_version(
        platform_name, owner=Constants.OWNED_BY_SELF
    )
    fileoperations.update_platform_version(version)
...
def _name_to_arn(platform_name):
    ...
    if PlatformRegExpressions.VALID_PLATFORM_NAME_FORMAT.match(platform_name):
        arn = platform_version_ops.get_platform_arn(
            platform_name, "latest", owner=Constants.OWNED_BY_SELF
        )
