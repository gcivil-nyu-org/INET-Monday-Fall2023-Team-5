...
def ssh_into_instance(
    instance_id, keep_open=False, force_open=False, custom_ssh=None, command=None
):
    ...
    try:
        ...
        if returncode != 0:
            ...
            raise CommandError(
                "An error occurred while running: " + custom_ssh[0] + "."
            )
    except OSError:
        ...
        if keep_open:
            ...
            (not has_restriction or force_open) and group_id and not rule_existed_before
        ):
...
def _generate_and_upload_keypair(keys):
    ...
    try:
        exitcode = subprocess.call(["ssh-keygen", "-f", file_name, "-C", keyname])
    except OSError:
