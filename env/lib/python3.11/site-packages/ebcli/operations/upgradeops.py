    ...
    if latest.name == env.platform.name:
        ...
        single = (
            elasticbeanstalk.get_option_setting(
                env.option_settings, namespaces.ENVIRONMENT, "EnvironmentType"
            )
            == "SingleInstance"
        )
        rolling_enabled = (
        ...
        warning = _get_warning_message(
            confirm, single, rolling_enabled, webserver, noroll
        )
        if warning:
...
def do_upgrade(
    ...
    if add_rolling:
        ...
        changes = [
            elasticbeanstalk.create_option_setting(
                namespaces.ROLLING_UPDATES, option_names.ROLLING_UPDATE_ENABLED, "true"
            ),
            elasticbeanstalk.create_option_setting(
                namespaces.ROLLING_UPDATES, option_names.ROLLING_UPDATE_TYPE, roll_type
            ),
        ]
    ...
    if PlatformVersion.is_valid_arn(platform_arn):
        commonops.update_environment(
            env_name, changes, None, timeout=timeout, platform_arn=platform_arn
        )
    else:
        commonops.update_environment(
            env_name,
