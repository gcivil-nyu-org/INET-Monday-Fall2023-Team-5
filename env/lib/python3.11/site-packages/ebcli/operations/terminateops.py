...
def terminate(env_name, force_terminate=False, nohang=False, timeout=15):
    request_id = elasticbeanstalk.terminate_environment(
        env_name, force_terminate=force_terminate
    )
...
def delete_app(app_name, force, nohang=False, cleanup=True, timeout=15):
    ...
    if not nohang:
        commonops.wait_for_success_events(
            request_id, sleep_time=5, timeout_in_minutes=timeout
        )
