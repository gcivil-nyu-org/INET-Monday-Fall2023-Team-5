...
def make_new_env(
    ...
    if fileoperations.build_spec_exists():
        ...
        LOG.debug(
            "Retrieved build configuration from buildspec: {0}".format(
    ...
    if not env_request.sample_application and not env_request.version_label:
        if source is not None:
            ...
            env_request.version_label = commonops.create_codecommit_app_version(
                env_request.app_name,
                process=process_app_version,
                build_config=build_config,
            )
            ...
            env_request.version_label = commonops.create_app_version(
                env_request.app_name,
                process=process_app_version,
                build_config=build_config,
            )
        ...
        if build_config is not None:
            ...
            success = commonops.wait_for_processed_app_versions(
                env_request.app_name, [env_request.version_label], timeout=timeout or 5
            )
            if not success:
                return

    if env_request.version_label is None or env_request.sample_application:
        env_request.version_label = commonops.create_dummy_app_version(
            env_request.app_name
        )
...
def should_download_sample_app():
    ...
    if heuristics.directory_is_empty():
        return io.get_boolean_response(
            text=strings["create.sample_application_download_option"], default=True
        )
...
def download_and_extract_sample_app(env_name):
    ...
    try:
        ...
        io.echo(
            "INFO: {}".format(strings["create.sample_application_download_complete"])
        )
...
def retrieve_application_version_url(env_name):
    ...
    try:
        ...
        io.log_warning(
            "{}. ".format(
                strings["cloudformation.cannot_find_app_source_for_environment"]
            )
        )
...
def create_default_service_role():
    ...
    """
    io.log_info(
        "Creating service role {} with default permissions.".format(
    ...
    try:
        iam.create_role_with_policy(
            role_name, trust_document, DEFAULT_SERVICE_ROLE_POLICIES
        )
    except NotAuthorizedError as e:
        raise NotAuthorizedError(
            prompts["create.servicerole.nopermissions"].format(
                DEFAULT_SERVICE_ROLE_NAME, e
            )
        )
...
def resolve_roles(env_request, interactive):
    ...
    if (
        not env_request.instance_profile
    ...
    if (
        ...
        if role is None:
            if interactive:
                ...
                input = io.get_input(prompts["create.servicerole.view"], default="")
