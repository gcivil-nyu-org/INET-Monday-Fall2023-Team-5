...
class ContainerFSHandler(object):
    ...
    def make_dockerfile(self):
        ...
        fileoperations.write_to_text_file(
            location=destination, data=dockerfile_contents
        )
    ...
    def copy_dockerfile(self, soln_stk, container_cfg):
        ...
        dfile_path = containerops._get_runtime_dockerfile_path(soln_stk, container_cfg)
...
class MultiContainerFSHandler(object):
    ...
    def make_docker_compose(self, env):
        ...
        compose_yaml = yaml.safe_dump(
            self._get_compose_dict(env, hostlog_path), default_flow_style=False
        )
        fileoperations.write_to_text_file(
            location=self.pathconfig.compose_path(), data=compose_yaml
        )

    def _get_compose_dict(self, env, hostlog_path):
        return compose.compose_dict(
            self.dockerrun, self.pathconfig.docker_proj_path(), hostlog_path, env
        )
...
def _append_dockerignore(dockerignore_path):
    ...
    fileoperations.append_to_text_file(dockerignore_path, dockerignore_contents)
