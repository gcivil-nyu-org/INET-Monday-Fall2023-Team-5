...
def run_container(
    ...
    _run_container(image_id, container_port, host_port, envvars_map, volume_map, name)
...
def get_exposed_hostports(container_id):
    ...
    try:
        ...
        return utils.flatten(
            [
                [p[HOST_PORT_KEY] for p in ports]
                for ports in six.itervalues(port_map)
                if ports
            ]
        )
...
def _run_container(image_id, container_port, host_port, envvars_map, volume_map, name):
...
def _get_container_port(full_docker_path):
    return _fst_match_in_dockerfile(
        full_docker_path,
        lambda s: s.startswith(EXPOSE_CMD),
        strings["local.run.noportexposed"],
    )[1]
...
def _get_opts(_map, opt_name, val_format):
    ...
    return utils.flatten([[opt_name, val_format.format(k, v)] for k, v in kv_pairs])
