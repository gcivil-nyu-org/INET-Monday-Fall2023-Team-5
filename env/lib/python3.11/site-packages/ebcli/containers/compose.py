...
def compose_dict(dockerrun, docker_proj_path, host_log, high_priority_env):
    ...
    services = {}
    definitions = dockerrun.get(CONTAINER_DEF_KEY, []) + dockerrun.get(
        LOCAL_CONTAINER_DEF_KEY, []
    )
...
def _add_service(services, definition, volume_map, host_log, high_priority_env):
    ...
    if links:
        service[COMPOSE_LINKS_KEY] = ["{}:{}".format(_fakename(n), n) for n in links]
    if merged_envvars:
...
def _get_definition_envvars(definition):
    return {
        e[CONTAINER_DEF_ENV_NAME_KEY]: e[CONTAINER_DEF_ENV_VALUE_KEY]
        for e in definition.get(CONTAINER_DEF_ENV_KEY, [])
    }
