...
class AbstractContainer(object):
    ...
    def __init__(
        self, fs_handler, soln_stk, container_cfg, opt_env=None, host_port=None
    ):
        """
    ...
    def _build(self):
        return commands.build_img(
            self.pathconfig.docker_proj_path(), self._get_full_docker_path()
        )
    ...
    def _run(self, img_id):
        ...
        if log_volume_map:
            log.make_logdirs(
                self.pathconfig.logdir_path(), utils.anykey(log_volume_map)
            )

        return commands.run_container(
            full_docker_path=self._get_full_docker_path(),
            image_id=img_id,
    ...
    def _get_log_volume_map(self):
        return log.get_log_volume_map(
            self.pathconfig.logdir_path(), self.fs_handler.dockerrun
        )
