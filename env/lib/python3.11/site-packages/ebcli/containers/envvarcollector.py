...
class EnvvarCollector(object):
    ...
    def filtered(self):
        ...
        filtered_envvars = {
            k: v for k, v in six.iteritems(self.map) if k not in self.to_remove
        }
    ...
    def merge(self, higher_priority_env):
        ...
        envvars_map = utils.merge_dicts(
            low_priority=self.map, high_priority=higher_priority_env.map
        )
