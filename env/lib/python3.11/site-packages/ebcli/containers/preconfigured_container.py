...
class PreconfiguredContainer(AbstractContainer):
    ...
    def validate(self):
        if self.pathconfig.dockerfile_exists():
            _validate_preconfig_dockerfile(
                self.soln_stk, self.container_cfg, self.pathconfig.dockerfile_path()
            )
    ...
    def _get_log_volume_map(self):
        ...
        if log_volume_map:  # User provided Logging in Dockerrun.aws.json
            ...
            cont_log = containerops.get_runtime_default_log_path(
                self.soln_stk, self.container_cfg
            )
            return {host_log: cont_log}


def _validate_preconfig_dockerfile(soln_stk, container_config, full_docker_path):
    """
    ...
    container_info = containerops._get_preconfig_info(soln_stk, container_config)
    expected_img = container_info[containerops.RUNTIME_IMG_KEY]
    actual_img = commands._get_base_img(full_docker_path)
    err_msg = "Invalid base Docker img in Dockerfile. Expected {} but got {}".format(
