...
def boot2docker_setup(env=os.environ):
    if not heuristics.is_boot2docker_installed():
        return
    LOG.debug(
        "Ensuring boot2docker VM has initialized, started and the client is set up..."
    )
    ...
    boot2docker_certs_path = os.path.sep.join(
    ...
    if DOCKER_CERT_PATH not in env:
        env[DOCKER_CERT_PATH] = os.path.join(
            fileoperations.get_home(), boot2docker_certs_path
        )
