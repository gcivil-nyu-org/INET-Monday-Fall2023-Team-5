...
class TagsController(AbstractBaseController):
    class Meta:
        ...
        arguments = [
            (
                ["environment_name"],
                dict(action="store", nargs="?", help=flag_text["tags.env"]),
            ),
            (["-l", "--list"], dict(action="store_true", help=flag_text["tags.list"])),
            (
                ["-a", "--add"],
                dict(
                    metavar="key1=value1[,key2=value2,...]", help=flag_text["tags.add"]
                ),
            ),
            (
                ["-d", "--delete"],
                dict(metavar="key1[,key2,...]", help=flag_text["tags.delete"]),
            ),
            (
                ["-u", "--update"],
                dict(
                    metavar="key1=value1[,key2=value2,...]",
                    help=flag_text["tags.update"],
                ),
            ),
    ...
    def do_command(self):
        ...
        tagops.handle_addition_string(
            self.add_arguments
        ) if self.add_arguments else None
        tagops.handle_deletion_string(
            self.delete_arguments
        ) if self.delete_arguments else None
        tagops.handle_update_string(
            self.update_arguments
        ) if self.update_arguments else None
