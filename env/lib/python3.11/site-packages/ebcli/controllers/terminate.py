...
class TerminateController(AbstractBaseController):
    class Meta:
        ...
        arguments = AbstractBaseController.Meta.arguments + [
            (["--force"], dict(action="store_true", help=flag_text["terminate.force"])),
            (
                ["--ignore-links"],
                dict(action="store_true", help=flag_text["terminate.ignorelinks"]),
            ),
            (["--all"], dict(action="store_true", help=flag_text["terminate.all"])),
            (
                ["-nh", "--nohang"],
                dict(action="store_true", help=flag_text["terminate.nohang"]),
            ),
    ...
    def do_command(self):
        ...
        if delete_application_and_resources:
            ...
            terminateops.delete_app(
                app_name, force, nohang=nohang, cleanup=cleanup, timeout=timeout
            )
            ...
            terminateops.terminate(
                env_name, force_terminate=ignore_links, nohang=nohang, timeout=timeout
            )
