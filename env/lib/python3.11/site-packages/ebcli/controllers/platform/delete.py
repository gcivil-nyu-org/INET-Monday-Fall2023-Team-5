...
class GenericPlatformDeleteController(AbstractBaseController):
    class Meta:
        ...
        arguments = [
            ...
            (
                ["--cleanup"],
                dict(action="store_true", help=flag_text["platformdelete.cleanup"]),
            ),
            (
                ["--all-platforms"],
                dict(
                    action="store_true", help=flag_text["platformdelete.allplatforms"]
                ),
            ),
            (
                ["--force"],
                dict(action="store_true", help=flag_text["platformdelete.force"]),
            ),
        ]
        ...
        if failed_versions:
            if not force:
                if not platform_name:
                    io.echo(
                        prompts["cleanupplatform.confirm"].replace(
                    ...
                    io.validate_action(
                        prompts["cleanupplatform.validate"], platform_name
                    )
