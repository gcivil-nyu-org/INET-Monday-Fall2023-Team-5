...
class PlatformShowController(AbstractBaseController):
    ...
    def do_command(self):
        ...
        if PlatformVersion.is_custom_platform_arn(config_platform):
            platform_name, latest_platform = self.get_latest_custom_platform(
                config_platform
            )
        else:
        ...
        if env_name:
            if want_solution_stack:
                platform = platformops.get_environment_platform(
                    app_name, env_name, want_solution_stack=want_solution_stack
                ).name
            else:
                platform = platformops.get_environment_platform(
                    app_name, env_name, want_solution_stack=want_solution_stack
            ...
            io.echo()
            io.echo(
                'Platform info for environment "{env_name}":'.format(env_name=env_name)
            )
    ...
    def get_latest_custom_platform(self, platform_arn):
        latest_platform = platform_version_ops.get_latest_custom_platform_version(
            platform_arn
        )
    ...
    def do_command(self):
        try:
            ...
            if not self.app.pargs.version:
                io.log_error(
                    "This workspace is currently associated with a deleted version."
                )
            else:
