...
class CreateController(AbstractBaseController):
    class Meta:
        ...
        arguments = [
            ...
            (["-i", "--instance_type"], dict(help=flag_text["create.itype"])),
            (["-p", "--platform"], dict(help=flag_text["create.platform"])),
            (
                ["-s", "--single"],
                dict(action="store_true", help=flag_text["create.single"]),
            ),
            (["--sample"], dict(action="store_true", help=flag_text["create.sample"])),
            (
                ["-d", "--branch_default"],
                dict(action="store_true", help=flag_text["create.default"]),
            ),
            (["-ip", "--instance_profile"], dict(help=flag_text["create.iprofile"])),
            (["-sr", "--service-role"], dict(help=flag_text["create.servicerole"])),
            ...
            (
                ["-nh", "--nohang"],
                dict(action="store_true", help=flag_text["create.nohang"]),
            ),
            ...
            (
                ["-lp", "--shared-lb-port"],
                dict(help=flag_text["create.shared_lb_port"]),
            ),
            (
                ["-es", "--enable-spot"],
                dict(action="store_true", help=flag_text["create.enable_spot"]),
            ),
            (["-sm", "--spot-max-price"], dict(help=flag_text["create.maxprice"])),
            (
                ["-it", "--instance-types"],
                dict(help=flag_text["create.instance_types"]),
            ),
            (
                ["-sb", "--on-demand-base-capacity"],
                dict(help=flag_text["create.on_demand_capacity"]),
            ),
            (
                ["-sp", "--on-demand-above-base-capacity"],
                dict(help=flag_text["create.on_demand_above_base_percent"]),
            ),
            (["-im", "--min-instances"], dict(help=flag_text["create.min_instances"])),
            (["-ix", "--max-instances"], dict(help=flag_text["create.max_instances"])),
            (
                ["-db", "--database"],
                dict(action="store_true", help=flag_text["create.database"]),
            ),
            # Hidden RDS commands
            (
                ["-db.user", "--database.username"],
                dict(dest="db_user", help=argparse.SUPPRESS),
            ),
            (
            ...
            (["--vpc"], dict(action="store_true", help=flag_text["create.vpc"])),
            (["--vpc.id"], dict(dest="vpc_id", help=argparse.SUPPRESS)),
            (["--vpc.ec2subnets"], dict(dest="vpc_ec2subnets", help=argparse.SUPPRESS)),
            (["--vpc.elbsubnets"], dict(dest="vpc_elbsubnets", help=argparse.SUPPRESS)),
            (
                ["--vpc.elbpublic"],
                dict(action="store_true", dest="vpc_elbpublic", help=argparse.SUPPRESS),
            ),
            (
                ["--vpc.publicip"],
                dict(action="store_true", dest="vpc_publicip", help=argparse.SUPPRESS),
            ),
            (
                ["--vpc.securitygroups"],
                dict(dest="vpc_securitygroups", help=argparse.SUPPRESS),
            ),
            (["--vpc.dbsubnets"], dict(dest="vpc_dbsubnets", help=argparse.SUPPRESS)),
            (
                ["-pr", "--process"],
                dict(action="store_true", help=flag_text["create.process"]),
            ),
        ]
    ...
    def do_command(self):
        ...
        if single and shared_lb:
            raise InvalidOptionsError(
                alerts["create.can_not_use_options_together"].format(
                    "--single", "--shared-lb"
                )
            )
        ...
        if tier and Tier.looks_like_worker_tier(tier):
            if (
                self.app.pargs.vpc_elbpublic
                or self.app.pargs.vpc_elbsubnets
                or self.app.pargs.vpc_publicip
            ):
                raise InvalidOptionsError(
        ...
        if (
            spot_max_price or on_demand_base_capacity or on_demand_above_base_capacity
        ) and not enable_spot:
        ...
        shared_lb = get_shared_load_balancer(
            interactive, elb_type, platform, shared_lb, vpc
        )
        shared_lb_port = (
            shared_lb_port
            or shared_lb_ops.get_shared_lb_port_from_customer(interactive, shared_lb)
        )
        enable_spot = enable_spot or spotops.get_spot_request_from_customer(interactive)
        instance_types = (
            instance_types
            or spotops.get_spot_instance_types_from_customer(interactive, enable_spot)
        )
    ...
    def form_database_object(self):
        ...
        if create_db or username or password or engine or size or instance or version:
            db_object = dict()
            if not username:
                io.echo()
                username = io.get_input(prompts["rds.username"], default="ebroot")
            if not password:
    ...
    def compose_multiple_apps(self):
        ...
        for module in module_names:
            if not os.path.isdir(os.path.join(root_dir, module)):
                io.log_warning(
                    strings["create.appdoesntexist"].replace("{app_name}", module)
                )
                continue
            ...
            if not fileoperations.env_yaml_exists():
                io.log_warning(strings["compose.noenvyaml"].replace("{module}", module))
                continue

            io.echo(
                "--- Creating application version for module: {0} ---".format(module)
            )
        ...
        if len(version_labels) > 0:
            composeops.compose(
                app_name, version_labels, grouped_env_names, group, nohang, timeout
            )
        else:
...
def get_elb_type_from_customer(interactive, single, tier):
    ...
    result = utils.prompt_for_item_in_list(
        [
            elb_names.CLASSIC_VERSION,
            elb_names.APPLICATION_VERSION,
...
def get_shared_load_balancer(interactive, elb_type, platform, shared_lb=None, vpc=None):
    if shared_lb:
        ...
        shared_lb = shared_lb_ops.get_shared_lb_from_customer(
            interactive, elb_type, platform, vpc
        )
...
def get_and_validate_envars(environment_variables_input):
    ...
    (
        environment_variable_option_settings,
        options_to_remove,
    ) = envvarops.create_environment_variables_list(environment_variables)
...
def _determine_platform(platform_string=None, iprofile=None):
    ...
    if isinstance(platform, PlatformVersion):
        ...
        if (
            platform.platform_branch_lifecycle_state
            == platform_branch_lifecycle_states.RETIRED
        ):
