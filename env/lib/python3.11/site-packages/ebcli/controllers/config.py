...
class ConfigController(AbstractBaseController):
    class Meta:
        ...
        arguments = [
            ...
            (
                ["-nh", "--nohang"],
                dict(action="store_true", help=flag_text["config.nohang"]),
            ),
            ...
            (
                ["-d", "--display"],
                dict(action="store_true", help=flag_text["config.display"]),
            ),
    ...
    def do_command(self):
        ...
        if display:
            configops.display_environment_configuration(
                app_name, env_name, output_format=output_format
            )
        elif update:
            configops.modify_environment_configuration(
                env_name, update, nohang, timeout
            )
        else:
            input_exists = not sys.stdin.isatty()
            if not cfg and not input_exists:
                configops.update_environment_configuration(
                    app_name, env_name, nohang, timeout=timeout
                )
                return
            ...
            if cfg:
                ...
                saved_configs.update_environment_with_config_file(
                    env_name, cfg_name, nohang, timeout=timeout
                )
            elif input_exists:
                data = sys.stdin.read()
                saved_configs.update_environment_with_config_data(
                    env_name, data, nohang, timeout=timeout
                )

    @expose(help="Save a configuration of the environment.")
    def save(self):
        cfg_name = self.app.pargs.cfg
        env_name = self.get_env_name(varname="name", cmd_example="eb config save")
        app_name = self.get_app_name()
    ...
    def get(self):
        ...
        try:
            ...
            io.log_error(strings["config.notfound"].replace("{config-name}", name))
