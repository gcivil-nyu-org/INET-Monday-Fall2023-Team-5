...
class DeployController(AbstractBaseController):
    class Meta(AbstractBaseController.Meta):
        ...
        arguments = [
            (
                ["environment_name"],
                dict(
                    action="store", nargs="?", default=[], help=flag_text["deploy.env"]
                ),
            ),
            ...
            (
                ["-nh", "--nohang"],
                dict(action="store_true", help=flag_text["deploy.nohang"]),
            ),
            (["--staged"], dict(action="store_true", help=flag_text["deploy.staged"])),
            (["--timeout"], dict(type=int, help=flag_text["general.timeout"])),
            (["--source"], dict(help=flag_text["deploy.source"])),
            (
                ["-p", "--process"],
                dict(action="store_true", help=flag_text["deploy.process"]),
            ),
        ]
    ...
    def multiple_app_deploy(self):
        ...
        if len(missing_env_yaml) > 0:
            ...
            io.echo(
                strings["deploy.modulemissingenvyaml"].replace(
                    "{modules}", module_list[:-2]
                )
            )
    ...
    def compose_deploy(self):
        ...
        for module in modules:
            if not path.isdir(path.join(top_dir, module)):
                io.log_error(
                    strings["deploy.notadirectory"].replace("{module}", module)
                )
                continue
            ...
            if environment_name is not None:
                commonops.set_environment_for_current_branch(
                    environment_name.replace("+", "-{0}".format(group_name))
                )
                ...
                stages_version_labels[group_name] = [
                    v for v in stages_version_labels[group_name] if v != version_label
                ]
        ...
        if len(stages_version_labels) > 0:
            for stage in stages_version_labels.keys():
                request_id = composeops.compose_no_events(
                    app_name, stages_version_labels[stage], group_name=stage
                )
                ...
                commonops.wait_for_compose_events(
                    request_id, app_name, env_names, self.timeout
                )
        else:
