...
class AppVersionController(AbstractBaseController):
    class Meta(AbstractBaseController.Meta):
        ...
        arguments = [
            ...
            (
                ["--create", "-c"],
                dict(action="store_true", help=flag_text["appversion.create"]),
            ),
            ...
            (
                ["--staged"],
                dict(action="store_true", help=flag_text["appversion.staged"]),
            ),
            (
                ["--timeout"],
                dict(default=5, type=int, help=flag_text["general.timeout"]),
            ),
    ...
    def do_command(self):
        ...
        if self.app.pargs.create:
            ...
            appversionops.create_app_version_without_deployment(
                self.app_name,
                self.label,
                self.staged,
                self.process,
        ...
        if self.app.pargs.delete is not None:
            ...
            appversionops.delete_app_version_label(
                self.app_name, version_label_to_delete
            )
            return
    ...
    def interactive_list_version(self):
        ...
        """
        app_versions = elasticbeanstalk.get_application_versions(self.app_name)[
            "ApplicationVersions"
        ]
