...
class InitController(AbstractBaseController):
    class Meta:
        ...
        arguments = [
            (
                ["application_name"],
                dict(help=flag_text["init.name"], nargs="?", default=[]),
            ),
            ...
            (
                ["-i", "--interactive"],
                dict(action="store_true", help=flag_text["init.interactive"]),
            ),
    ...
    def do_command(self):
        ...
        force_non_interactive = _customer_is_avoiding_interactive_flow(self.app.pargs)
        ...
        configure_keyname(
            platform,
            keyname,
            keyname_of_existing_application,
            interactive,
            force_non_interactive,
        )
            ...
            if os.path.exists(module) and os.path.isdir(module):
                ...
                if not application_created:
                    (
                        platform_arn,
                        keyname_of_existing_application,
                    ) = commonops.create_app(app_name, default_env=default_env)
                    application_created = True
                else:
                    (
                        platform_arn,
                        keyname_of_existing_application,
                    ) = commonops.pull_down_app_info(app_name, default_env=default_env)
...
def get_repository_interactive():
    ...
    current_repository = (
        current_repository or fileoperations.get_current_directory_name()
    )
    ...
    if len(repo_list) > 0:
        ...
        repo_name = utils.prompt_for_item_in_list(repo_list, default=default_option)
...
def create_codecommit_branch(source_control, branch_name):
    ...
    if current_commit is None:
        ...
        if not staged_files:
            ...
            LOG.debug(
                "Cannot create placeholder commit because there are staged files: {0}".format(
                    staged_files
                )
            )
            io.echo(
...
def get_branch_interactive(repository):
    ...
    if len(branch_list) > 0:
        ...
        branch_name = utils.prompt_for_item_in_list(branch_list, default=default_option)
...
def configure_codecommit(source):
    ...
    if not source_location:
        should_continue = io.get_boolean_response(
            text=prompts["codecommit.usecc"], default=True
        )
...
def configure_keyname(
    solution, keyname, keyname_of_existing_app, interactive, force_non_interactive
):
    if "IIS" not in solution:
        keyname = get_keyname(
            keyname, keyname_of_existing_app, interactive, force_non_interactive
        )
...
def get_keyname(keyname, keyname_of_existing_app, interactive, force_non_interactive):
    ...
    if (interactive and not keyname_passed_through_command_line) or (
        not keyname and not force_non_interactive
    ):
...
def handle_buildspec_image(solution, force_non_interactive):
    ...
    if not force_non_interactive and build_spec and build_spec.image is None:
        LOG.debug(
            "Buildspec file is present but image does not exist. Attempting to fill best guess."
        )
        platform_image = initializeops.get_codebuild_image_from_platform(solution)

        if not platform_image:
            io.echo(
                "No images found for platform: {platform}".format(platform=solution)
            )
            return None
        ...
        if isinstance(platform_image[0], dict):
            if len(platform_image) == 1:
                io.echo(
                    strings["codebuild.latestplatform"].replace("{platform}", solution)
                )
                selected_image = platform_image[0]
            else:
                io.echo(
                    prompts["codebuild.getplatform"].replace("{platform}", solution)
                )
                selected = int(
                    utils.prompt_for_index_in_list(
                        [image["description"] for image in platform_image]
                    )
                )
                ...
                matching_images = [
                    image
                    for image in platform_image
                    if selected_description == image["description"]
                ]

                if not matching_images:
                    LOG.error(
                        f"No matching images found for selected description: {selected}"
                    )
                    return None

                selected_image = matching_images[0]
            fileoperations.write_buildspec_config_header(
...
def establish_codecommit_repository_and_branch(
    repository, branch, source_control, source_location
):
    repository = establish_codecommit_repository(
        repository, source_control, source_location
    )
    branch = establish_codecommit_branch(
        repository, branch, source_control, source_location
    )
...
def _determine_platform(
    ...
    if not force_interactive:
        ...
        if not platform and fileoperations.env_yaml_exists():
            ...
            if platform:
                io.echo(
                    strings["init.usingenvyamlplatform"].replace("{platform}", platform)
                )
