...
class LogsController(AbstractBaseController):
    class Meta:
        ...
        arguments = AbstractBaseController.Meta.arguments + [
            (["-a", "--all"], dict(action="store_true", help=flag_text["logs.all"])),
            (["-z", "--zip"], dict(action="store_true", help=flag_text["logs.zip"])),
            (["-i", "--instance"], dict(help=flag_text["logs.instance"])),
            (["-g", "--log-group"], dict(help=flag_text["logs.log-group"])),
            (
                ["-cw", "--cloudwatch-logs"],
                dict(
                    help=flag_text["logs.cloudwatch_logs"],
                    nargs="?",
            ...
            (
                ["-cls", "--cloudwatch-log-source"],
                dict(help=flag_text["logs.cloudwatch_log_source"]),
            ),
    ...
    def __get_logs(self):
        ...
        if (
            ...
            raise InvalidOptionsError(
                strings["logs.cloudwatch_log_source_argumnent_is_invalid_for_retrieval"]
            )
    ...
    def __modify_log_streaming(self):
        ...
        if self.cloudwatch_logs in ["enable", None]:
            logsops.enable_cloudwatch_logs(
                self.app_name, self.env_name, self.cloudwatch_log_source
            )
        elif self.cloudwatch_logs == "disable":
            logsops.disable_cloudwatch_logs(
                self.app_name, self.env_name, self.cloudwatch_log_source
            )

    def __normalized_log_group_name(self):
        return logsops.normalize_log_group_name(
            self.env_name, self.log_group, self.cloudwatch_log_source
        )
    ...
    def __stream_cloudwatch_logs(self):
        ...
        if (
            ...
            if not logsops.instance_log_streaming_enabled(self.app_name, self.env_name):
                raise InvalidOptionsError(
                    strings["logs.instance_log_streaming_disabled"].format(
                        self.env_name
                    )
                )
        ...
        if (
            ...
            if not logsops.environment_health_streaming_enabled(
                self.app_name, self.env_name
            ):
                raise InvalidOptionsError(
                    strings["logs.environment_health_log_streaming_disabled"].format(
                        self.env_name
                    )
                )
