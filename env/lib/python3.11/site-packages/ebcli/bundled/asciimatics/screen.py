...
from ebcli.bundled.asciimatics.exceptions import (
    ResizeScreenError,
    StopApplication,
    NextScene,
)
...
class _DoubleBuffer(object):
    ...
    def __init__(self, height, width):
        ...
        line = [(ord(" "), Screen.COLOUR_WHITE, 0, 0, 1) for _ in range(self._width)]
    ...
    def clear(self, fg, attr, bg):
        ...
        """
        line = [(ord(" "), fg, attr, bg, 1) for _ in range(self._width)]
...
class _AbstractCanvas(with_metaclass(ABCMeta, object)):
        ...
        if len(text) > 0:
            ...
            for i, c in enumerate(text):
                ...
                if c != " " or not transparent:
                    ...
                    if (
                        x + i + j + 1 < self.width
                        and self._buffer.get(x + i + j + 1, y)[4] == 0
                    ):
                        self._buffer.set(x + i + j + 1, y, (ord("x"), 0, 0, 0, 1))
    ...
    def paint(
        self, text, x, y, colour=7, attr=0, bg=0, transparent=False, colour_map=None
    ):
        """
    ...
    def _blend(self, new, old, ratio):
        ...
        for c in range(self.colours):
            ...
            diff = sqrt(
                ((rc - r) * 0.3) ** 2 + ((gc - g) * 0.59) ** 2 + ((bc - b) * 0.11) ** 2
            )
            if diff < nearest:
                nearest = diff
                match = c
    ...
    def is_visible(self, x, y):
        ...
        return (
            (x >= 0)
            and (x <= self.width)
            and (y >= self._start_line)
            and (y < self._start_line + self.height)
        )
    ...
    def draw(self, x, y, char=None, colour=7, bg=0, thin=False):
        ...
        if (
            (x0 < 0 and x1 < 0)
            or (x0 >= self.width * 2 and x1 >= self.width * 2)
            or (y0 < 0 and y1 < 0)
        ...
        def _fast_fill(start_x, end_x, iy):
            ...
            for ix in range(start_x, end_x):
                ...
                if ix % 2 == 1:
                    self.print_at(
                        line_chars[next_char], ix // 2, iy // 2, colour, bg=bg
                    )
            if end_x % 2 == 1:
        ...
        def _draw_on_x(ix, iy):
            ...
            while ix != x1:
                ...
                if char is None:
                    self.print_at(
                        line_chars[next_char], px // 2, py // 2, colour, bg=bg
                    )
                else:
    ...
    def fill_polygon(self, polygons, colour=7, bg=0):
        ...
        for polygon in polygons:
            ...
            if (
                p_max_x < 0
                or p_min_x >= self.width
                or p_max_y < 0
                or p_min_y > self.height
            ):
                continue
                ...
                if 0 <= y < self.height:
                    if edge.max_y != y:
                        ...
                        if count % 2 == 1:
                            ...
                            if not (
                                (last_x < 0 and edge.x < 0)
                                or (last_x >= self.width and edge.x >= self.width)
                            ):
                                # Clip raster to screen width.
                                self.move(max(0, last_x), y)
                                self.draw(
                                    min(edge.x, self.width),
                                    y,
                                    colour=colour,
                                    bg=bg,
                                    thin=True,
            ...
            for x in range(self.width):
                ...
                if c[4] != 0:
                    self._screen.print_at(
                        chr(c[0]), x + self._dx, y + self._dy, c[1], c[2], c[3]
                    )
...
class Screen(with_metaclass(ABCMeta, _AbstractCanvas)):
    ...
    def __init__(self, height, width, buffer_height, unicode_aware):
        ...
        """
        super(Screen, self).__init__(height, width, buffer_height, 0, unicode_aware)
    ...
    def open(cls, height=200, catch_interrupt=False, unicode_aware=None):
        ...
        if sys.platform == "win32":
            ...
            screen = _WindowsScreen(
                win_out, win_in, height, old_out, in_mode, unicode_aware=unicode_aware
            )
        else:
    ...
    @classmethod
    def wrapper(
        cls, func, height=200, catch_interrupt=False, arguments=None, unicode_aware=None
    ):
        """
        ...
        """
        screen = Screen.open(
            height, catch_interrupt=catch_interrupt, unicode_aware=unicode_aware
        )
        restore = True
        try:
            try:
        ...
        if self._last_start_line != self._start_line:
            ...
            if self._start_line > self._last_start_line:
                self._buffer.invalidate(
                    self._last_start_line + self.height, self._start_line + self.height
                )
            else:
                ...
                if self.has_resized():
                    if stop_on_resize:
                        ...
                        raise ResizeScreenError(
                            "Screen resized", self._scenes[self._scene_index]
                        )
                b = time.time()
                if b - a < 0.05:
                    if allow_int:
        ...
        try:
            ...
            if got_event or self._idle_frame_count <= 0 or self._forced_update:
                ...
                for effect in scene.effects:
                    ...
                    if effect.frame_update_count > 0:
                        self._idle_frame_count = min(
                            self._idle_frame_count, effect.frame_update_count
                        )
                self.refresh()
            ...
            if e.name is None:
                ...
                for i, scene in enumerate(self._scenes):
                    ...
                    raise RuntimeError("Could not find Scene: '{}'".format(e.name))
...
if sys.platform == "win32":
    ...
    class _WindowsScreen(Screen):
        ...
        _COLOURS = {
            ...
            Screen.COLOUR_MAGENTA: (
                win32console.FOREGROUND_RED | win32console.FOREGROUND_BLUE
            ),
            Screen.COLOUR_CYAN: (
                win32console.FOREGROUND_BLUE | win32console.FOREGROUND_GREEN
            ),
            Screen.COLOUR_WHITE: (
        ...
        _BG_COLOURS = {
            ...
            Screen.COLOUR_YELLOW: (
                win32console.BACKGROUND_RED | win32console.BACKGROUND_GREEN
            ),
            Screen.COLOUR_BLUE: win32console.BACKGROUND_BLUE,
            Screen.COLOUR_MAGENTA: (
                win32console.BACKGROUND_RED | win32console.BACKGROUND_BLUE
            ),
            Screen.COLOUR_CYAN: (
                win32console.BACKGROUND_BLUE | win32console.BACKGROUND_GREEN
            ),
            Screen.COLOUR_WHITE: (
        ...
        def __init__(
            self, stdout, stdin, buffer_height, old_out, old_in, unicode_aware=False
        ):
            """
        ...
        def get_event(self):
            ...
            while len(self._stdin.PeekConsoleInput(1)) > 0:
                ...
                if event.EventType == win32console.KEY_EVENT:
                    ...
                    return MouseEvent(
                        event.MousePosition.X, event.MousePosition.Y, button
                    )
    ...
    class _CursesScreen(Screen):
        ...
        def __init__(self, win, height=200, catch_interrupt=False, unicode_aware=False):
            """
        ...
        def _scroll(self, lines):
            ...
            if lines < 0:
                ...
                self._safe_write(
                    "{}{}".format(
                        curses.tparm(self._move_y_x, self.height, 0).decode("utf-8"),
                ...
                if key == curses.KEY_RESIZE:
                    ...
                    if self._unicode_aware and key > 0:
                        if key & 0xC0 == 0xC0:
                            ...
                            logger.debug(
                                "Byte stream: %d bytes left", self._bytes_to_read
                            )
                            continue
        ...
        def set_title(self, title):
            ...
            if self._start_line is not None:
                self._safe_write(
                    "{}{}{}".format(self._start_title, title, self._end_title)
                )
