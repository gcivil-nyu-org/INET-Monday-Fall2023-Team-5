...
class CementPluginHandler(plugin.CementPluginHandler):
    ...
    def _setup(self, app_obj):
        ...
        config_handler = self.app.handler.get("config", self.app.config._meta.label)
        ...
        for config_dir in self.config_dirs:
            ...
            if not os.path.exists(config_dir):
                LOG.debug("plugin config dir %s does not exist." % config_dir)
                continue
            else:
                ...
                for config in plugin_config_files:
                    ...
                    if is_true(pconfig.get(plugin, "enable_plugin")):
                        LOG.debug("enabling plugin '%s' per plugin config" % plugin)
                        ...
                        LOG.debug("disabling plugin '%s' per plugin config" % plugin)
    ...
    def _load_plugin_from_dir(self, plugin_name, plugin_dir):
        ...
        LOG.debug("attempting to load '%s' from '%s'" % (plugin_name, plugin_dir))
    ...
    def _load_plugin_from_bootstrap(self, plugin_name, base_package):
        ...
        LOG.debug("attempting to load '%s' from '%s'" % (plugin_name, base_package))
        # We don't catch this because it would make debugging a nightmare
