...
class HandlerManager(object):
    ...
    def get(self, handler_type, handler_label, *args):
        ...
        if handler_type not in self.__handlers__:
            raise exc.FrameworkError("handler type '%s' does not exist!" % handler_type)
        ...
        if handler_label in self.__handlers__[handler_type]:
            ...
            raise exc.FrameworkError(
                "handlers['%s']['%s'] does not exist!" % (handler_type, handler_label)
            )
    ...
    def define(self, interface):
        ...
        if not hasattr(interface, "IMeta"):
            raise exc.InterfaceError(
                "Invalid %s, " % interface + "missing 'IMeta' class."
            )
        if not hasattr(interface.IMeta, "label"):
            raise exc.InterfaceError(
                "Invalid %s, " % interface + "missing 'IMeta.label' class."
            )

        LOG.debug(
            "defining handler type '%s' (%s)"
            % (interface.IMeta.label, interface.__name__)
        )

        if interface.IMeta.label in self.__handlers__:
            raise exc.FrameworkError(
                "Handler type '%s' already defined!" % interface.IMeta.label
            )
    ...
    def register(self, handler_obj):
        ...
        if not hasattr(obj._meta, "label") or not obj._meta.label:
            raise exc.InterfaceError(
                "Invalid handler %s, " % orig_obj + "missing '_meta.label'."
            )
        if not hasattr(obj._meta, "interface") or not obj._meta.interface:
            raise exc.InterfaceError(
                "Invalid handler %s, " % orig_obj + "missing '_meta.interface'."
            )
        ...
        if handler_type not in self.__handlers__:
            raise exc.FrameworkError("Handler type '%s' doesn't exist." % handler_type)
        if (
            obj._meta.label in self.__handlers__[handler_type]
            and self.__handlers__[handler_type][obj._meta.label] != obj
        ):
            raise exc.FrameworkError(
                "handlers['%s']['%s'] already exists" % (handler_type, obj._meta.label)
            )
        ...
        if hasattr(interface.IMeta, "validator"):
            ...
            LOG.debug(
                "Interface '%s' does not have a validator() function!" % interface
            )
    ...
    def registered(self, handler_type, handler_label):
        ...
        """
        if (
            handler_type in self.__handlers__
            and handler_label in self.__handlers__[handler_type]
        ):
            return True
    ...
    def _setup(self, app_obj):
        ...
        if self._meta.config_section is None:
            self._meta.config_section = "%s.%s" % (
                self._meta.interface.IMeta.label,
                self._meta.label,
            )
...
def define(interface):
    ...
    LOG.debug(
        "defining handler type '%s' (%s)" % (interface.IMeta.label, interface.__name__)
    )
...
def register(handler_obj):
    ...
    if handler_type not in backend.__handlers__:
        raise exc.FrameworkError("Handler type '%s' doesn't exist." % handler_type)
    if (
        obj._meta.label in backend.__handlers__[handler_type]
        and backend.__handlers__[handler_type][obj._meta.label] != obj
    ):
        raise exc.FrameworkError(
            "handlers['%s']['%s'] already exists" % (handler_type, obj._meta.label)
        )
    ...
    if hasattr(interface.IMeta, "validator"):
        ...
        LOG.debug("Interface '%s' does not have a validator() function!" % interface)
...
def registered(handler_type, handler_label):
    ...
    if (
        handler_type in backend.__handlers__
        and handler_label in backend.__handlers__[handler_type]
    ):
        return True
...
def resolve(handler_type, handler_def, raise_error=True):
    ...
    msg = "Unable to resolve handler '%s' of type '%s'" % (handler_def, handler_type)
    if han is not None:
        return han
    elif han is None and raise_error:
        raise exc.FrameworkError(msg)
