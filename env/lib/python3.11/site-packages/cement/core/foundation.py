...
def add_handler_override_options(app):
    ...
    for i in app._meta.handler_override_options:
        if i not in interface.list():
            LOG.debug(
                "interface '%s'" % i + " is not defined, can not override handlers"
            )
            continue
        ...
        if len(app.handler.list(i)) > 1:
            ...
            if not len(choices) > 0:
                LOG.debug(
                    "no handlers are overridable within the " + "%s interface" % i
                )
                continue
...
class CementApp(meta.MetaMixin):
    ...
    def extend(self, member_name, member_object):
        ...
        LOG.debug("extending appication with '.%s' (%s)" % (member_name, member_object))
            ...
            for member in dir(self._parsed_args):
                ...
                for section in self.config.get_sections():
                    if member in self.config.keys(section):
                        self.config.set(
                            section, member, getattr(self._parsed_args, member)
                        )
    ...
    def _setup_arg_handler(self):
        ...
        self.args.add_argument(
            "--debug", dest="debug", action="store_true", help="toggle debug output"
        )
        self.args.add_argument(
            "--quiet",
    ...
    def _setup_controllers(self):
        ...
        if self._meta.base_controller is not None:
            cntr = self._resolve_handler("controller", self._meta.base_controller)
        ...
        if self._meta.base_controller is not None:
            if self._meta.base_controller._meta.label != "base":
                raise exc.FrameworkError(
                    "Base controllers must have " + "a label of 'base'."
                )
