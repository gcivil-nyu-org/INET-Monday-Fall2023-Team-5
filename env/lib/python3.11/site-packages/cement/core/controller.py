...
def controller_validator(klass, obj):
    ...
    if not obj._meta.label == "base" and obj._meta.stacked_on is None:
        errmsg = "Controller `%s` is not stacked anywhere!" % obj.__class__.__name__
        raise exc.InterfaceError(errmsg)
    if not obj._meta.label == "base" and obj._meta.stacked_type not in [
        "nested",
        "embedded",
    ]:
        raise exc.InterfaceError(
            "Controller '%s' " % obj._meta.label
            + "has an unknown stacked type of '%s'." % obj._meta.stacked_type
        )
...
class CementBaseController(handler.CementBaseHandler):
    ...
    def _setup(self, app_obj):
        ...
        if getattr(self._meta, "description", None) is None:
            self._meta.description = "%s Controller" % self._meta.label.capitalize()
    ...
    def _dispatch(self):
        ...
        if self._dispatch_command:
            if self._dispatch_command["func_name"] == "_dispatch":
                ...
                self._parse_args()
                func = getattr(
                    self._dispatch_command["controller"],
    ...
    def _usage_text(self):
        ...
        txt = "%s (sub-commands ...) [options ...] {arguments ...}" % self.app.args.prog
        return txt
